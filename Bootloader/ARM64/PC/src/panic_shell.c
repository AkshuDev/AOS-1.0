...
EFIAPI static void execute_cmd(struct ambrc* ambrc, struct VMemDesign* design, aos_bool* running) {
    if (cmd_len == 0) return;
    ...
    switch (c[0]) {
        ...
        case 'c':
            if (!strcmp(c, "clear")) {
                ...
                vmem_print("CPU Architecture: ARM\n");
                valid = AOS_TRUE;
            }
            break;
...
EFIAPI void start_panic_shell(struct drive_device* current_drive, const char* err, size_t err_lines) {
    ...
    size_t start_x = (uefi_width / 2) - 8;
    size_t start_y = 0;
    
    size_t tstart_x = 0;
    size_t tstart_y = 2+err_lines;
    size_t t_w = uefi_width;
    size_t t_h = uefi_height - 2;
    ...
    while (running) {
        char c = read_input();
        if (!((c >= 32 && c <= 126) || c == '\r' || c == '\b')) continue;
        switch (c) {
            case '\b': {
                if (cmd_len == 0) break;
                cmd[--cmd_len] = ' ';
