...
EFIAPI EFI_STATUS btl_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE* SystemTable) {
	uint64_t tsc_start = timer_read_cntvct();
    ...
    uint64_t tsc_start2 = timer_read_cntvct();
    kdelay(10);
    uint64_t tsc_end = timer_read_cntvct();
