...
uint8_t pcie_init() {
    ...
    for (int i = 0; i < mcfg_num_segs; i++) {
        ...
        uint64_t virt_base = BTL_PCIE_BASE + ((uint64_t)e->pcie_segment << 28);
    }
    return 1;
}
...
uint32_t pcie_read(uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset) {
    if (mcfg_table) {
        ...
        if (eidx >= 0 && eidx < mcfg_num_segs) {
            ...
            uint64_t virt_addr = (uint64_t)(BTL_PCIE_BASE + ((uint64_t)e->pcie_segment << 28) + (((uint64_t)bus - e->start_bus) << 20) + ((uint64_t)slot << 15) + ((uint64_t)func << 12) + (offset));
