#include <aos_inttypes.h>
#include <asm.h>
#include <system.h>

#include <inc/mm/avmf.h>
#include <inc/mm/pager.h>

#include <inc/drivers/gpu/virtio.h>
#include <inc/drivers/io/io.h>
#include <inc/drivers/gpu/gpu.h>
#include <inc/core/pcie.h>
#include <inc/core/smp.h>
#include <inc/core/module.h>
#include <inc/core/kfuncs.h>

#define KVIRTIO_ALLOC_STEP 16

#define MAX_CMD_RESP_BUFS 16

typedef struct {
	struct virtqueue virtq;
	aos_bool acceleration_present;

	struct virtio_gpu_ctrl_hdr* cmd_buf;
	struct virtio_gpu_resp_display_info* resp_buf;
	uint64_t cmd_buf_phys[MAX_CMD_RESP_BUFS];
	uint64_t resp_buf_phys[MAX_CMD_RESP_BUFS];
	aos_bool buf_busy[MAX_CMD_RESP_BUFS];
	uint8_t* desc_to_buf;

	spinlock_t virtq_lock;

	uintptr_t notify_base;
	uint32_t notify_multiplier;

	uint32_t gpu_cmd_core;

	volatile struct virtio_common_cfg* common_cfg;
	uint64_t mapping_size;

	uint64_t idx;
	struct gpu_device* gpu;
	aos_bool valid;
} virtio_controller;

static virtio_controller* controllers;
static uint64_t controller_count;
static uint64_t controller_cap;

static void wait_till_requests_are_complete(virtio_controller* kvc) {

}

static uint16_t wait_for_free_buf(virtio_controller* kvc) {
	for (;;) {
        for (uint16_t i = 0; i < MAX_CMD_RESP_BUFS; i++) {
            uint64_t flags = spin_lock_irqsave(&kvc->virtq_lock);

            if (!kvc->buf_busy[i]) {
                kvc->buf_busy[i] = 1;
                spin_unlock_irqrestore(&kvc->virtq_lock, flags);
                return i;
            }

            spin_unlock_irqrestore(&kvc->virtq_lock, flags);
        }

        __asm__ volatile("pause");
    }
}

static aos_bool virtio_reset_device(virtio_controller* kvc) {
	if (!kvc) return AOS_FALSE;
	if (kvc->valid || kvc->gpu->active) return AOS_FALSE; // NEVER REINITIALIZE ON WORKING DEVICE
	
	kvc->common_cfg->device_status = 0;
	__asm__ volatile("mfence" ::: "memory");
	uint64_t timeout = kget_ms_passed();
    while (kvc->common_cfg->device_status != 0) {
		if (kget_ms_passed() - timeout > 10000) {
			serial_print("[VirtIO:GPU] Reset timed out!\n");
			return AOS_FALSE;
		}
		__asm__ volatile("pause");
	}
    kvc->common_cfg->device_status |= VIRTIO_STATUS_ACKNOWLEDGE;
	__asm__ volatile("mfence" ::: "memory");
    kvc->common_cfg->device_status |= VIRTIO_STATUS_DRIVER;
	__asm__ volatile("mfence" ::: "memory");
	kdelay_us(300);

	serial_print("[VirtIO:GPU] Device Reset Complete!\n");
	return AOS_TRUE;
}

static aos_bool virtio_negotiate_features(virtio_controller* kvc) {
	if (!kvc) return AOS_FALSE;
	if (kvc->valid || kvc->gpu->active) return AOS_FALSE; // NEVER REINITIALIZE ON WORKING DEVICE

	kvc->common_cfg->device_feature_select = 0;
	__asm__ volatile("mfence" ::: "memory");
	kdelay_us(300);
  	uint32_t dev_lo = kvc->common_cfg->device_feature;

	kvc->common_cfg->device_feature_select = 1;
	__asm__ volatile("mfence" ::: "memory");
	kdelay_us(300);
	uint32_t dev_hi = kvc->common_cfg->device_feature;

	uint64_t device_features = ((uint64_t)dev_hi << 32) | dev_lo;

	if (device_features == 0x0 || device_features == 0xFFFFFFFFFFFFFFFFULL) {
		serial_printf("[VirtIO:GPU] Invalid features provided! (0x%llx)\n", device_features);
		return AOS_FALSE;
	}

	uint64_t driver_features = 0;

	if (device_features & (1ULL << VIRTIO_F_VERSION_1)) driver_features |= 1ULL << VIRTIO_F_VERSION_1;
	else {
		serial_print("[VirtIO:GPU] Device failed to provide VIRTIO_F_VERSION_1 as a feature\n");
		return AOS_FALSE;
	}
	if (device_features & (1ULL << VIRTIO_GPU_F_VIRGL)) {
		driver_features |= 1ULL << VIRTIO_GPU_F_VIRGL;
		kvc->gpu->acceleration_present = AOS_TRUE;
		kvc->acceleration_present = AOS_TRUE;
	} else {
		kvc->gpu->acceleration_present = AOS_FALSE;
		kvc->acceleration_present = AOS_FALSE;
	}

	kvc->common_cfg->driver_feature_select = 0;
	__asm__ volatile("mfence" ::: "memory");
	kdelay_us(300);
	kvc->common_cfg->driver_feature = (uint32_t)driver_features;
	__asm__ volatile("mfence" ::: "memory");
	kdelay_us(300);

	kvc->common_cfg->driver_feature_select = 1;
	__asm__ volatile("mfence" ::: "memory");
	kdelay_us(300);

	kvc->common_cfg->driver_feature = (uint32_t)(driver_features >> 32);
	__asm__ volatile("mfence" ::: "memory");

    kvc->common_cfg->device_status |= VIRTIO_STATUS_FEATURES_OK;
	__asm__ volatile("mfence" ::: "memory");
	kdelay_us(300);
    if (!(kvc->common_cfg->device_status & VIRTIO_STATUS_FEATURES_OK)) {
        serial_print("[VirtIO:GPU] Failed to negotiate features!\n");
        return AOS_FALSE;
    }
	serial_print("[VirtIO:GPU] Features negotiated successfully!\n");
	return AOS_TRUE;
}

static aos_bool virtio_setup_queues(virtio_controller* kvc, uint16_t q_idx) {
	if (!kvc) return AOS_FALSE;
	if (kvc->valid || kvc->gpu->active) return AOS_FALSE; // NEVER REINITIALIZE ON WORKING DEVICE
	
	if (kvc->common_cfg->num_queues < 1) {
		serial_print("[VirtIO:GPU] Device has no queues, quiting...\n");
		return AOS_FALSE;
	}

    serial_print("[VirtIO:GPU] Setting Queues....\n");

    kvc->common_cfg->queue_select = q_idx;
	__asm__ volatile("mfence" ::: "memory");
	kdelay_us(300);

	uint16_t size = kvc->common_cfg->queue_size;
	kvc->common_cfg->queue_msix_vector = 0xFFFF;
	kvc->common_cfg->queue_size = size;
	__asm__ volatile("mfence" ::: "memory");

	if (size < 1) {
		serial_print("[VirtIO:GPU] CommonCFG Queue Size is less than 1, quiting...\n");
		return AOS_FALSE;
	}

    size_t desc_t_size = ALIGN_UP(size * sizeof(struct virtq_desc), 16);
    size_t avail_r_size = ALIGN_UP(6 + (2 * size), 4); // better to keep 4 than 2
    size_t used_r_size = ALIGN_UP(6 + (8 * size), 4);

    uint64_t phys = 0;
    void* mem = (void*)avmf_alloc(desc_t_size + avail_r_size + used_r_size, MALLOC_TYPE_DRIVER, AVMF_FLAG_RW | AVMF_FLAG_NO_CACHE, &phys);
    if (!mem) {serial_print("[VirtIO:GPU] Failed to Allocate Memory!\n"); return AOS_FALSE;}
    if (!phys) {serial_print("[VirtIO:GPU] Failed to retrieve physical address!\n"); return AOS_FALSE;}

	memset(mem, 0, desc_t_size + avail_r_size + used_r_size);

    uint64_t flags = spin_lock_irqsave(&kvc->virtq_lock);
    kvc->virtq.desc = (volatile struct virtq_desc*)mem;
    kvc->virtq.avail = (volatile struct virtq_avail*)((uintptr_t)mem + desc_t_size);
    kvc->virtq.used = (volatile struct virtq_used*)((uintptr_t)kvc->virtq.avail + avail_r_size);
    kvc->virtq.queue_size = size;
    kvc->virtq.free_head = 0;
	kvc->virtq.last_used_idx = 0;

	uint64_t desc_count = desc_t_size / sizeof(struct virtq_desc);
	for (uint64_t i = 0; i < desc_count; i++) {
		if (i + 1 == desc_count) {
			kvc->virtq.desc[i].next = 0xFFFF;
			break;
		}
		kvc->virtq.desc[i].next = i + 1;
	}

    spin_unlock_irqrestore(&kvc->virtq_lock, flags);

	kvc->desc_to_buf = (uint8_t*)avmf_alloc(sizeof(uint8_t) * desc_count, MALLOC_TYPE_DRIVER, AVMF_FLAG_RW, NULL);
    if (!kvc->desc_to_buf) {serial_print("[VirtIO:GPU] Failed to Allocate Memory!\n"); return AOS_FALSE;}
    
    kvc->common_cfg->queue_desc = phys;
    kvc->common_cfg->queue_avail = phys + desc_t_size;
    kvc->common_cfg->queue_used = (uintptr_t)phys + avail_r_size + desc_t_size;
    kvc->common_cfg->queue_enable = 1;
	__asm__ volatile("mfence" ::: "memory");
	kdelay_us(300);

	if (kvc->common_cfg->queue_enable != 1) {
		serial_print("[VirtIO:GPU] Device rejected queue!\n");
		return AOS_FALSE;
	}

    serial_print("[VirtIO:GPU] Queues ready!\n");
	return AOS_TRUE;
}

static void virtio_destroy_queues(virtio_controller* kvc) {
	if (!kvc) return;
	if (kvc->virtq.desc) {
		wait_till_requests_are_complete(kvc);

		uint64_t flags = spin_lock_irqsave(&kvc->virtq_lock);
		avmf_free((uint64_t)kvc->virtq.desc);
		kvc->virtq.desc = NULL;
		spin_unlock_irqrestore(&kvc->virtq_lock, flags);
	}
	serial_print("[VirtIO:GPU] Queues Destroyed!\n");
}

static aos_bool virtio_setup_buffers(virtio_controller* kvc) {
	if (!kvc) return AOS_FALSE;
	if (kvc->valid || kvc->gpu->active) return AOS_FALSE; // NEVER REINITIALIZE ON WORKING DEVICE

	uint64_t cmd_phys = 0;
	uint64_t resp_phys = 0;
	kvc->cmd_buf = (struct virtio_gpu_ctrl_hdr*)avmf_alloc(0x1000, MALLOC_TYPE_DRIVER, AVMF_FLAG_RW | AVMF_FLAG_NO_CACHE, &cmd_phys);
	kvc->resp_buf = (struct virtio_gpu_resp_display_info*)avmf_alloc(0x1000, MALLOC_TYPE_DRIVER, AVMF_FLAG_RW | AVMF_FLAG_NO_CACHE, &resp_phys);
	if (!kvc->cmd_buf || !kvc->resp_buf || !cmd_phys || !resp_phys) {
		serial_print("[VirtIO:GPU] Failed to allocate command and response buffers!\n");
		return AOS_FALSE;
	}

	for (uint8_t i = 0; i < MAX_CMD_RESP_BUFS; i++) {
		kvc->cmd_buf_phys[i] = cmd_phys + (i * sizeof(struct virtio_gpu_ctrl_hdr));
		kvc->resp_buf_phys[i] = resp_phys + (i * sizeof(struct virtio_gpu_resp_display_info));
	}

	serial_print("[VirtIO:GPU] Buffers Created!\n");
	return AOS_TRUE;
}

static void virtio_destroy_buffers(virtio_controller* kvc) {
	if (!kvc) return;

	if (kvc->cmd_buf) {
		avmf_free((uint64_t)kvc->cmd_buf);
		kvc->cmd_buf = NULL;
		memset(kvc->cmd_buf_phys, 0, sizeof(uint64_t) * MAX_CMD_RESP_BUFS);
	}
		
	if (kvc->resp_buf) {
		avmf_free((uint64_t)kvc->resp_buf);
		kvc->resp_buf = NULL;
		memset(kvc->resp_buf_phys, 0, sizeof(uint64_t) * MAX_CMD_RESP_BUFS);
	}

	serial_print("[VirtIO:GPU] Buffers Destroyed!\n");
}

static void virtio_release_cmd_core(virtio_controller* kvc) {
	if (!kvc) return;

	if (kvc->gpu_cmd_core != 0xFFFF) {
		enum core_status status;
		if (!smp_get_core_status(kvc->gpu_cmd_core, &status)) return;
		uint64_t timeout = kget_ms_passed();
		aos_bool invalid_core = AOS_FALSE;
		while (status != CORE_STATUS_RESERVED) {
			switch (status) {
				case CORE_STATUS_READY: {
					// CMD Core Invalid
					invalid_core = AOS_TRUE;
					break;
				}
				default: break;
			}
			if (invalid_core) break;
			if (kget_timestamp_ms() - timeout > 15000) {
				// Drop core, no other choice
				smp_reset_core(kvc->gpu_cmd_core);
			}
		}
		if (!invalid_core) {
			smp_unreserve_core(kvc->gpu_cmd_core);
			serial_printf("[VirtIO:GPU] Unreserved Core %u which was reserved for sending ASYNC GPU Commands\n", kvc->gpu_cmd_core);
		} else {
			serial_printf("[VirtIO:GPU] Core %u is invalid, already unreserved!\n", kvc->gpu_cmd_core);
		}
	} else {
		serial_print("[VirtIO:GPU] No Core to unreserve\n");
	}
}

static aos_bool virtio_setup_cmd_core(virtio_controller* kvc) {
	if (!kvc) return AOS_FALSE;
	if (kvc->gpu_cmd_core != 0xFFFF) virtio_release_cmd_core(kvc);

	kvc->gpu_cmd_core = 0xFFFF;
	return AOS_FALSE; // TODO: Fix Multi Core Submissions
	smp_get_first_free_core(&kvc->gpu_cmd_core);
	if (kvc->gpu_cmd_core != 0xFFFF) {
		smp_reserve_core(kvc->gpu_cmd_core);
		serial_printf("[VirtIO:GPU] Reserved Core %u for sending ASYNC GPU Commands\n", kvc->gpu_cmd_core);
	} else {
		serial_print("[VirtIO:GPU] No Core available for reservation!\n");
	}
	return AOS_TRUE;
}

static void virtqueue_wait_complete_async(void* kvc_raw) {
	virtio_controller* kvc = (virtio_controller*)kvc_raw;
	if (!kvc) {
		smp_yield();
		return;
	}
	if (!kvc->valid) {
		smp_yield();
		return;
	}

	uint16_t head_desc = kvc->virtq.avail->ring[(kvc->virtq.avail->idx-1) % kvc->virtq.queue_size];
	uint8_t pbuf_slot = kvc->desc_to_buf ? kvc->desc_to_buf[head_desc] : 0xFF;

	if (pbuf_slot < MAX_CMD_RESP_BUFS) kvc->buf_busy[pbuf_slot] = AOS_TRUE;

	uint64_t timeout = kget_timestamp_ms();
    while (kvc->virtq.used->idx == kvc->virtq.last_used_idx) {
		if (kget_timestamp_ms() - timeout >= 10000) {
			serial_print("[VirtIO:GPU] Timeout!\n");
			smp_yield();
			return;
		}
        __asm__ volatile("pause");
    }

    uint64_t flags = spin_lock_irqsave(&kvc->virtq_lock);

	uint16_t head = kvc->virtq.used->ring[kvc->virtq.last_used_idx % kvc->virtq.queue_size].id;
	uint16_t tail = head;

	while (kvc->virtq.desc[tail].flags & VIRTQ_DESC_F_NEXT) tail = kvc->virtq.desc[tail].next;

	while (kvc->virtq.last_used_idx != kvc->virtq.used->idx) {
		struct virtq_used_elem* elem = (struct virtq_used_elem*)&kvc->virtq.used->ring[kvc->virtq.last_used_idx % kvc->virtq.queue_size];

		uint16_t head = elem->id;
		uint16_t tail = head;

		while (kvc->virtq.desc[tail].flags & VIRTQ_DESC_F_NEXT) tail = kvc->virtq.desc[tail].next;

		kvc->virtq.desc[tail].next = kvc->virtq.free_head;
		kvc->virtq.free_head = head;

		kvc->virtq.last_used_idx++;
	}

	if (pbuf_slot < MAX_CMD_RESP_BUFS) kvc->buf_busy[pbuf_slot] = AOS_FALSE;

    spin_unlock_irqrestore(&kvc->virtq_lock, flags);
    smp_yield();
}

static aos_bool virtqueue_wait_complete_sync(virtio_controller* kvc) {
	if (!kvc) return AOS_FALSE;

	uint16_t head_desc = kvc->virtq.avail->ring[(kvc->virtq.avail->idx-1) % kvc->virtq.queue_size];
	uint8_t pbuf_slot = kvc->desc_to_buf ? kvc->desc_to_buf[head_desc] : 0xFF;

	if (pbuf_slot < MAX_CMD_RESP_BUFS) kvc->buf_busy[pbuf_slot] = AOS_TRUE;

	uint64_t timeout = kget_timestamp_ms();
    while (kvc->virtq.used->idx == kvc->virtq.last_used_idx) {
		if (kget_timestamp_ms() - timeout >= 10000) {
			serial_print("[VirtIO:GPU] Timeout!\n");
			return AOS_FALSE;
		}
        __asm__ volatile("pause");
    }

    uint64_t flags = spin_lock_irqsave(&kvc->virtq_lock);

	uint16_t head = kvc->virtq.used->ring[kvc->virtq.last_used_idx % kvc->virtq.queue_size].id;
	uint16_t tail = head;

	while (kvc->virtq.desc[tail].flags & VIRTQ_DESC_F_NEXT) tail = kvc->virtq.desc[tail].next;

	while (kvc->virtq.last_used_idx != kvc->virtq.used->idx) {
		struct virtq_used_elem* elem = (struct virtq_used_elem*)&kvc->virtq.used->ring[kvc->virtq.last_used_idx % kvc->virtq.queue_size];

		uint16_t head = elem->id;
		uint16_t tail = head;

		while (kvc->virtq.desc[tail].flags & VIRTQ_DESC_F_NEXT) tail = kvc->virtq.desc[tail].next;

		kvc->virtq.desc[tail].next = kvc->virtq.free_head;
		kvc->virtq.free_head = head;

		kvc->virtq.last_used_idx++;
	}
    
	if (pbuf_slot < MAX_CMD_RESP_BUFS) kvc->buf_busy[pbuf_slot] = AOS_FALSE;
    spin_unlock_irqrestore(&kvc->virtq_lock, flags);

    return AOS_TRUE;
}

static aos_bool virtio_send_ex(virtio_controller* kvc, struct virtq_desc* descs, size_t desc_count, uint16_t* head_idx_out) {
	if (!kvc) return AOS_FALSE;

    uint64_t flags = spin_lock_irqsave(&kvc->virtq_lock);

	if (kvc->virtq.free_head == 0xFFFF) {
		spin_unlock_irqrestore(&kvc->virtq_lock, flags);
		return AOS_FALSE;
	}

	uint16_t avail_idx = kvc->virtq.avail->idx;
    uint16_t head = kvc->virtq.free_head;
	kvc->virtq.free_head = kvc->virtq.desc[head].next;
	if (head_idx_out) *head_idx_out = head;

	uint16_t c = head;
    for (size_t i = 0; i < desc_count; i++) {
		uint16_t idx = c;
		memcpy((void*)&kvc->virtq.desc[idx], &descs[i], sizeof(struct virtq_desc));
		if (i + 1 < desc_count) { // Continue chain
			if (kvc->virtq.free_head == 0xFFFF) {
				spin_unlock_irqrestore(&kvc->virtq_lock, flags);
				return AOS_FALSE;
			}
			c = kvc->virtq.free_head;
			kvc->virtq.free_head = kvc->virtq.desc[c].next;

			kvc->virtq.desc[idx].next = c; // Link
			kvc->virtq.desc[idx].flags |= VIRTQ_DESC_F_NEXT;
		} else {
			kvc->virtq.desc[idx].next = 0xFFFF; // Pause
			if (kvc->virtq.desc[idx].flags & VIRTQ_DESC_F_NEXT) kvc->virtq.desc[idx].flags &= ~VIRTQ_DESC_F_NEXT;
		}
	}

    kvc->virtq.avail->ring[avail_idx % kvc->virtq.queue_size] = head;
    __asm__ volatile("mfence" ::: "memory"); // Ensure GPU sees RAM update
    kvc->virtq.avail->idx++;
	__asm__ volatile("mfence" ::: "memory"); // Ensure GPU sees RAM update
    spin_unlock_irqrestore(&kvc->virtq_lock, flags);

	return AOS_TRUE;
}

static aos_bool virtio_notify_device(virtio_controller* kvc) {
	if (!kvc) return AOS_FALSE;

	__asm__ volatile("mfence" ::: "memory");

	// Notify Doorbell
    uintptr_t db = kvc->notify_base + (kvc->common_cfg->queue_notify_off * kvc->notify_multiplier);
    *(volatile uint32_t*)db = 0;

	return AOS_TRUE;
}

static aos_bool virtio_submit_async(virtio_controller* kvc, uint8_t cmd_slot, size_t cmd_size, size_t resp_size) {
	if (!kvc) return AOS_FALSE;
	if (!kvc->cmd_buf || !kvc->desc_to_buf) return AOS_FALSE;
	if (cmd_slot > MAX_CMD_RESP_BUFS) return AOS_FALSE;
	uint64_t cmd_phys = kvc->cmd_buf_phys[cmd_slot];
	uint64_t resp_phys = kvc->resp_buf_phys[cmd_slot];

	struct virtq_desc descs[2] = {
		(struct virtq_desc){
			.addr = cmd_phys,
			.flags = VIRTQ_DESC_F_NEXT,
			.len = cmd_size,
			.next = 0 // Auto fills it
		},
		(struct virtq_desc){
			.addr = resp_phys,
			.flags = VIRTQ_DESC_F_WRITE,
			.len = resp_size,
			.next = 0xFFFF
		}
	};

	uint16_t head = 0;
    if (!virtio_send_ex(kvc, (struct virtq_desc*)descs, 2, &head)) return AOS_FALSE;
	kvc->desc_to_buf[head] = cmd_slot;

    // Poll for completion
    enum core_status status = 0;
    smp_get_core_status(kvc->gpu_cmd_core, &status);

	aos_bool out = AOS_TRUE;
    if (kvc->gpu_cmd_core != 0xFFFF && status == CORE_STATUS_RESERVED) {
        smp_push_task(kvc->gpu_cmd_core, virtqueue_wait_complete_async, kvc); // Push task here
		virtio_notify_device(kvc);
	} else {
		virtio_notify_device(kvc);
        out = virtqueue_wait_complete_sync(kvc); // either no extra core, or core busy
	}
	return out;
}

static aos_bool virtio_submit_sync(virtio_controller* kvc, uint8_t cmd_slot, size_t cmd_size, size_t resp_size) {
    if (!kvc) return AOS_FALSE;
	if (!kvc->cmd_buf) return AOS_FALSE;
	if (cmd_slot > MAX_CMD_RESP_BUFS) return AOS_FALSE;
	uint64_t cmd_phys = kvc->cmd_buf_phys[cmd_slot];
	uint64_t resp_phys = kvc->resp_buf_phys[cmd_slot];

	struct virtq_desc descs[2] = {
		(struct virtq_desc){
			.addr = cmd_phys,
			.flags = VIRTQ_DESC_F_NEXT,
			.len = cmd_size,
			.next = 0 // Auto fills it
		},
		(struct virtq_desc){
			.addr = resp_phys,
			.flags = VIRTQ_DESC_F_WRITE,
			.len = resp_size,
			.next = 0xFFFF
		}
	};

    uint16_t head = 0;
    if (!virtio_send_ex(kvc, (struct virtq_desc*)descs, 2, &head)) return AOS_FALSE;
	kvc->desc_to_buf[head] = cmd_slot;

    // Poll for completion
	virtio_notify_device(kvc);
    return virtqueue_wait_complete_sync(kvc);
}

static struct virtio_cap get_cap(uint8_t b, uint8_t s, uint8_t f, uint8_t target_type, aos_bool* valid) {
    struct virtio_cap cap = {0};
    uint8_t cap_ptr = pcie_read(b, s, f, 0x34) & 0xFF;
	*valid = AOS_FALSE;

    while (cap_ptr != 0) {
        uint32_t hdr = pcie_read(b, s, f, cap_ptr);
        uint8_t cap_id = hdr & 0xFF;
		uint8_t cap_next = (hdr >> 8) & 0xFF;
		uint8_t cfg_type = (hdr >> 24) & 0xFF;

        if (cap_id == 0x09) { // Vendor Specific
			uint32_t barinfo = pcie_read(b, s, f, cap_ptr + 4);
			uint8_t bar = barinfo & 0xFF;

            if (cfg_type == target_type && bar < 6) {
                cap.cap_ptr = cap_ptr;
                cap.bar = bar;
                cap.offset = pcie_read(b, s, f, cap_ptr + 8);
				cap.length = pcie_read(b, s, f, cap_ptr + 12);
				*valid = AOS_TRUE;
                return cap;
            }
        }
        cap_ptr = cap_next;
    }
    return cap;
}

static aos_bool virtio_map(virtio_controller* kvc) {
	if (!kvc) return AOS_FALSE;

	uint8_t bus = kvc->gpu->pcie_device->bus;
    uint8_t slot = kvc->gpu->pcie_device->slot;
    uint8_t func = kvc->gpu->pcie_device->func;

	aos_bool valid = AOS_FALSE;
	struct virtio_cap common_cap = get_cap(bus, slot, func, 1, &valid);
	if (!valid) {
		serial_print("[VirtIO:GPU] Invailid CAP\n");
		return AOS_FALSE;
	}

	aos_bool notify_valid = AOS_FALSE;
    struct virtio_cap notify_cap = get_cap(bus, slot, func, 2, &notify_valid);
    if (!notify_valid) {
        serial_print("[VirtIO:GPU] Invalid Notify CFG CAP\n");
        return AOS_FALSE;
    }

    uint32_t bar = pcie_read_bar(bus, slot, func, common_cap.bar);
    uint64_t bar_phys = bar & ~0xFULL;
	if ((bar_phys & 0xFFF) != 0) { // BAR should be page-aligned
		serial_print("[VirtIO:GPU] BAR is not page-aligned, mapping failed!\n");
		return AOS_FALSE;
	}

    aos_bool is_64bit = ((bar >> 1) & 0b011) == 0x2;
	uint32_t orig0 = bar;
    uint32_t orig1 = 0;

	if ((bar & 0b001) == 0x1) {
		serial_print("[VirtIO:GPU] BAR is not a memory BAR, mapping failed!\n");
		return AOS_FALSE;
	}

	if (is_64bit) {
        orig1 = pcie_read_bar(bus, slot, func, common_cap.bar+1);
        bar_phys |= ((uint64_t)orig1 << 32);
    }

	uint32_t notify_bar = pcie_read_bar(bus, slot, func, notify_cap.bar);
    uint64_t notify_bar_phys = notify_bar & ~0xFULL;
	if ((notify_bar_phys & 0xFFF) != 0) { // BAR should be page-aligned
		serial_print("[VirtIO:GPU] BAR is not page-aligned, mapping failed!\n");
		return AOS_FALSE;
	}

    aos_bool notify_is_64bit = ((notify_bar >> 1) & 0b011) == 0x2;
	uint32_t notify_orig0 = notify_bar;
    uint32_t notify_orig1 = 0;

	if ((notify_bar & 0b001) == 0x1) {
		serial_print("[VirtIO:GPU] BAR is not a memory BAR, mapping failed!\n");
		return AOS_FALSE;
	}

	if (notify_is_64bit) {
        notify_orig1 = pcie_read_bar(bus, slot, func, notify_cap.bar+1);
        notify_bar_phys |= ((uint64_t)notify_orig1 << 32);
    }

	aos_bool memspace = pcie_get_memory_space_toggled(bus, slot, func);
	if (memspace) pcie_toggle_memory_space(bus, slot, func, AOS_FALSE);

    pcie_write_bar(bus, slot, func, common_cap.bar, 0xFFFFFFFF);
    if (is_64bit) pcie_write_bar(bus, slot, func, common_cap.bar+1, 0xFFFFFFFF);

    uint32_t mask0 = pcie_read_bar(bus, slot, func, common_cap.bar);
    uint32_t mask1 = is_64bit ? pcie_read_bar(bus, slot, func, common_cap.bar+1) : 0;

    pcie_write_bar(bus, slot, func, common_cap.bar, orig0);
    if (is_64bit) pcie_write_bar(bus, slot, func, common_cap.bar+1, orig1);

	pcie_write_bar(bus, slot, func, notify_cap.bar, 0xFFFFFFFF);
    if (notify_is_64bit) pcie_write_bar(bus, slot, func, notify_cap.bar+1, 0xFFFFFFFF);

    uint32_t notify_mask0 = pcie_read_bar(bus, slot, func, notify_cap.bar);
    uint32_t notify_mask1 = notify_is_64bit ? pcie_read_bar(bus, slot, func, notify_cap.bar+1) : 0;

    pcie_write_bar(bus, slot, func, notify_cap.bar, notify_orig0);
    if (notify_is_64bit) pcie_write_bar(bus, slot, func, notify_cap.bar+1, notify_orig1);

	if (memspace) pcie_toggle_memory_space(bus, slot, func, AOS_TRUE);

	uint64_t size = 0;
	if (is_64bit) {
		uint64_t mask = (mask0 & ~0xFULL) | ((uint64_t)mask1 << 32);
		size = ~mask + 1;
	} else {
		uint32_t mask = (uint32_t)(mask0 & ~0xFULL);
		size = (uint32_t)(~mask + 1);
	}

	uint64_t notify_size = 0;
	if (notify_is_64bit) {
		uint64_t notify_mask = (notify_mask0 & ~0xFULL) | ((uint64_t)notify_mask1 << 32);
		notify_size = ~notify_mask + 1;
	} else {
		uint32_t notify_mask = (uint32_t)(notify_mask0 & ~0xFULL);
		notify_size = (uint32_t)(~notify_mask + 1);
	}

    kvc->mapping_size = size;
	if (kvc->mapping_size < sizeof(struct virtio_common_cfg)) {
		serial_print("[VirtIO:GPU] Device reported memory size is lower than minimum, mapping failed!\n");
		return AOS_FALSE;
	}

	pager_map_range(AOS_DIRECT_MAP_BASE + bar_phys, bar_phys, kvc->mapping_size, PAGE_PRESENT | PAGE_RW | PAGE_PCD);
	pager_map_range(AOS_DIRECT_MAP_BASE + notify_bar_phys, notify_bar_phys, notify_size, PAGE_PRESENT | PAGE_RW | PAGE_PCD);

	pcie_toggle_busmaster(bus, slot, func, AOS_TRUE);

	kvc->common_cfg = (volatile struct virtio_common_cfg*)(AOS_DIRECT_MAP_BASE + bar_phys + common_cap.offset);
	serial_printf("[VirtIO:GPU] Mapped all VIRTIO COMMON CFG (%llx (Phys) - Size: 0x%llx)\n", bar_phys, kvc->mapping_size);
	
    kvc->notify_base = (uintptr_t)(AOS_DIRECT_MAP_BASE + notify_bar_phys + notify_cap.offset);
    kvc->notify_multiplier = pcie_read(bus, slot, func, notify_cap.cap_ptr + 16);
	serial_printf("[VirtIO:GPU] Mapped all VIRTIO NOTIFY BASE and MULTIPLIER (%llx (Phys) - Size: 0x%llx)\n", notify_bar_phys, notify_size);

	return AOS_TRUE;
}

static void virtio_destroy(virtio_controller* kvc) {
	if (!kvc) return;

	if (kvc->common_cfg) {
		serial_print("[VirtIO:GPU] Releasing Device...\n");
		kvc->common_cfg->device_status |= VIRTIO_STATUS_FAILED;
		__asm__ volatile("mfence" ::: "memory");
		serial_print("[VirtIO:GPU] Device has been released!\n");
	}

	serial_print("[VirtIO:GPU] Unmapping Data...\n");
	virtio_destroy_buffers(kvc);
	virtio_destroy_queues(kvc);
	virtio_release_cmd_core(kvc);
	serial_print("[VirtIO:GPU] Unmap complete!\n[VirtIO:GPU] Destorying controller...\n");

	kvc->valid = AOS_FALSE;
	if (kvc->idx == controller_count-1) controller_count--;
	serial_print("[VirtIO:GPU] Controller destroyed!\n");
}

aos_bool virtio_flush(struct gpu_device* gpu_, uint32_t x, uint32_t y, uint32_t w, uint32_t h, int resource_id) {
    if (gpu_->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	virtio_controller* kvc = &controllers[gpu_->controller_idx];
	
    uint64_t cur_buf_slot = wait_for_free_buf(kvc);

    struct virtio_gpu_resource_flush* f = (struct virtio_gpu_resource_flush*)&kvc->cmd_buf[cur_buf_slot];
    memset(f, 0, sizeof(*f));
    f->hdr.type = VIRTIO_GPU_CMD_RESOURCE_FLUSH;
    f->r.x = x;
    f->r.y = y;
    f->r.width = w;
    f->r.height = h;
    f->resource_id = resource_id;
    f->padding = 0;

    return virtio_submit_async(kvc, cur_buf_slot, sizeof(*f), sizeof(struct virtio_gpu_ctrl_hdr));
}

// Total hours wasted on virtio: 18 (I DID COOK)
aos_bool virtio_init(struct AOS_Module* m) {
	if (m->hdr.type != MODULE_TYPE_DRIVER) return AOS_FALSE;
    if (m->Modules.driver_module.type != MODULE_DRIVER_TYPE_GPU) return AOS_FALSE;

	if (!controllers) {
		controllers = (virtio_controller*)avmf_alloc(sizeof(virtio_controller) * KVIRTIO_ALLOC_STEP, MALLOC_TYPE_DRIVER, AVMF_FLAG_RW, NULL);
		if (!controllers) return AOS_FALSE;
		controller_cap = KVIRTIO_ALLOC_STEP;
		controller_count = 0;
	} else if (controller_count >= controller_cap) {
		virtio_controller* nptr = (virtio_controller*)avmf_alloc(sizeof(virtio_controller) * (controller_cap + KVIRTIO_ALLOC_STEP), MALLOC_TYPE_DRIVER, AVMF_FLAG_RW, NULL);
		if (!nptr) return AOS_FALSE;
		memcpy(nptr, controllers, sizeof(virtio_controller)*controller_count);
		avmf_free((uint64_t)controllers);
		controllers = nptr;
		controller_cap += KVIRTIO_ALLOC_STEP;
	}

	struct gpu_device* gpu = &m->Modules.driver_module.DriverConnections.gpu_connector;
	virtio_controller* kvc = &controllers[controller_count];
	memset(kvc, 0, sizeof(virtio_controller));
	kvc->idx = controller_count;
	kvc->gpu = gpu;
	kvc->valid = AOS_FALSE;
	kvc->gpu_cmd_core = 0xFFFF;
	gpu->controller_idx = controller_count;
	controller_count++;

    serial_print("[VirtIO:GPU] Initializing...\n");
	if (!virtio_map(kvc)) {
		virtio_destroy(kvc);
		return AOS_FALSE;
	}
    
    // RESET
    if (!virtio_reset_device(kvc)) {
		virtio_destroy(kvc);
		return AOS_FALSE;
	}

    // Features
    if (!virtio_negotiate_features(kvc)) {
		virtio_destroy(kvc);
		return AOS_FALSE;
	}

    // Queues
    if (!virtio_setup_queues(kvc, 0)) {
		virtio_destroy(kvc);
		return AOS_FALSE;
	}
    
    // setup buffers
	if (!virtio_setup_buffers(kvc)) {
		virtio_destroy(kvc);
		return AOS_FALSE;
	}

    kvc->common_cfg->device_status |= VIRTIO_STATUS_DRIVER_OK;
	__asm__ volatile("mfence" ::: "memory");
	kdelay_us(300);

	uint8_t final_status = kvc->common_cfg->device_status;
	if ((final_status & VIRTIO_STATUS_FAILED) || !(final_status & VIRTIO_STATUS_DRIVER_OK)) {
        serial_printf("[VirtIO:GPU] Device rejected DRIVER_OK initialization status! (Status: 0x%x)\n", final_status);
        virtio_destroy(kvc);
        return AOS_FALSE;
    }

    // Setup gpu core
	if (!virtio_setup_cmd_core(kvc)) {
		kvc->gpu_cmd_core = 0xFFFF;
	}

    // Get fb info
    uint64_t cur_buf_slot = wait_for_free_buf(kvc);

    struct virtio_gpu_ctrl_hdr* get_info = (struct virtio_gpu_ctrl_hdr*)&kvc->cmd_buf[cur_buf_slot];
    memset(get_info, 0, sizeof(*get_info));
    get_info->type = VIRTIO_GPU_CMD_GET_DISPLAY_INFO;
    if (!virtio_submit_sync(kvc, cur_buf_slot, sizeof(*get_info), sizeof(struct virtio_gpu_resp_display_info))) {
		serial_print("[VirtIO:GPU] Failed to submit display command, device cannot be used!\n");
		virtio_destroy(kvc);
		return AOS_FALSE;
	}
    struct virtio_gpu_resp_display_info* display = (struct virtio_gpu_resp_display_info*)&kvc->resp_buf[cur_buf_slot];

    if (display->displays[0].enabled) {
        gpu->framebuffer->w = display->displays[0].r.width;
        gpu->framebuffer->h = display->displays[0].r.height;
    } else {
        // Fallback if the device doesn't have a preference
        gpu->framebuffer->w = 1024;
        gpu->framebuffer->h = 768;
    }
    gpu->framebuffer->bpp = 32;
    gpu->framebuffer->pitch = gpu->framebuffer->w * (gpu->framebuffer->bpp / 8);
    gpu->framebuffer->size = gpu->framebuffer->pitch * gpu->framebuffer->h;

	gpu->framebuffer->virt = avmf_alloc(gpu->framebuffer->size, MALLOC_TYPE_SENSITIVE, AVMF_FLAG_RW | AVMF_FLAG_NO_CACHE, &gpu->framebuffer->phys);

    gpu->active = AOS_TRUE;
    serial_print("[VirtIO:GPU] Initialization completed!\n");
	kvc->valid = AOS_TRUE;
	return AOS_TRUE;
}

aos_bool virtio_init_resources(struct gpu_device* gpu_, int id) {
	if (gpu_->controller_idx >= controller_count || !controllers) return AOS_FALSE;

    serial_print("[VirtIO:GPU] Initializing Resources...\n");
    serial_print("[VirtIO:GPU] Initialization of resources completed!\n");
	return AOS_TRUE;
}

aos_bool virtio_set_mode(struct gpu_device* gpu, uint32_t w, uint32_t h, uint32_t bpp) {
    (void)gpu; (void)w; (void)h; (void)bpp;
	return AOS_TRUE;
}

aos_bool virtio_switch_off(struct gpu_device* gpu_) {
	if (gpu_->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	virtio_controller* kvc = &controllers[gpu_->controller_idx];
	struct gpu_device* gpu = kvc->gpu; // ensure we use the linked gpu
	kvc->valid = AOS_FALSE;
	gpu->active = AOS_FALSE;

    serial_print("[VirtIO:GPU] Switching Off...\n");
    if (!virtio_reset_device(kvc)) {
		kvc->valid = AOS_TRUE;
		gpu->active = AOS_TRUE;
		return AOS_FALSE;
	}
	serial_print("[VirtIO:GPU] GPU Reset completed!\n[VirtIO:GPU] Unmapping used memory!\n");

	virtio_destroy_buffers(kvc);
	virtio_destroy_queues(kvc);
	virtio_release_cmd_core(kvc);
    serial_print("[VirtIO:GPU] Unmapping completed!\n");

    serial_print("[VirtIO:GPU] Switched off!\n");
	return AOS_TRUE;
}

aos_bool virtio_refresh(struct gpu_device* gpu_, uint64_t flags) {
	if (gpu_->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	virtio_controller* kvc = &controllers[gpu_->controller_idx];
	struct gpu_device* gpu = kvc->gpu; // ensure we use the linked gpu

	// Start of by refreshing gpu core
	if (flags & GPU_REFRESH_FLAG_CORE) {
		virtio_release_cmd_core(kvc);
		virtio_setup_cmd_core(kvc);
	}

	// Now refresh device
	if (flags & GPU_REFRESH_FLAG_BUFFERS) {
		uint64_t phys = 0;
		struct virtio_gpu_ctrl_hdr* buf = (struct virtio_gpu_ctrl_hdr*)avmf_alloc(MAX_CMD_RESP_BUFS * sizeof(struct virtio_gpu_ctrl_hdr), MALLOC_TYPE_DRIVER, AVMF_FLAG_RW | AVMF_FLAG_NO_CACHE, &phys);
		if (buf && phys) {
			avmf_free((uint64_t)kvc->cmd_buf);
			kvc->cmd_buf = buf;
			for (uint8_t i = 0; i < MAX_CMD_RESP_BUFS; i++) {
				kvc->cmd_buf_phys[i] = phys + (sizeof(struct virtio_gpu_ctrl_hdr) * i);
			}
		}

		phys = 0;
		struct virtio_gpu_resp_display_info* buf2 = (struct virtio_gpu_resp_display_info*)avmf_alloc(MAX_CMD_RESP_BUFS * sizeof(struct virtio_gpu_ctrl_hdr), MALLOC_TYPE_DRIVER, AVMF_FLAG_RW | AVMF_FLAG_NO_CACHE, &phys);
		if (buf2 && phys) {
			avmf_free((uint64_t)kvc->resp_buf);
			kvc->resp_buf = buf2;
			for (uint8_t i = 0; i < MAX_CMD_RESP_BUFS; i++) {
				kvc->resp_buf_phys[i] = phys + (sizeof(struct virtio_gpu_resp_display_info) * i);
			}
		}
	}

	if ((flags & GPU_REFRESH_FLAG_QUEUES) || (flags & GPU_REFRESH_FLAG_DEVICE)) {
		// Reset everything since we cannot reset only queues as they are live
		wait_till_requests_are_complete(kvc);

		kvc->valid = AOS_FALSE;
		kvc->gpu->active = AOS_FALSE;
		if (!virtio_reset_device(kvc)) {
			serial_print("[VirtIO:GPU] Refresh Failed!\n");
			kvc->valid = AOS_TRUE;
			kvc->gpu->active = AOS_TRUE;
		}

		// Just unmap memory here since everything after this is unrevertable
		virtio_destroy_queues(kvc);

		if (!virtio_negotiate_features(kvc)) {
			serial_print("[VirtIO:GPU] Refresh Failed! [Cannot Revert changes!]\n");
			virtio_destroy(kvc);
			return AOS_FALSE;
		}
		if (!virtio_setup_queues(kvc, 0)) {
			serial_print("[VirtIO:GPU] Refresh Failed! [Cannot Revert changes!]\n");
			virtio_destroy(kvc);
			return AOS_FALSE;
		}
		
		kvc->common_cfg->device_status |= VIRTIO_STATUS_DRIVER_OK;
		__asm__ volatile("mfence" ::: "memory");
		kdelay_us(300);

		uint8_t final_status = kvc->common_cfg->device_status;
		if ((final_status & VIRTIO_STATUS_FAILED) || !(final_status & VIRTIO_STATUS_DRIVER_OK)) {
			serial_print("[VirtIO:GPU] Refresh Failed! [Cannot Revert Changes]\n");
			virtio_destroy(kvc);
			return AOS_FALSE;
		}

		// Get fb info
		uint64_t cur_buf_slot = wait_for_free_buf(kvc);

		struct virtio_gpu_ctrl_hdr* get_info = (struct virtio_gpu_ctrl_hdr*)&kvc->cmd_buf[cur_buf_slot];
		memset(get_info, 0, sizeof(*get_info));
		get_info->type = VIRTIO_GPU_CMD_GET_DISPLAY_INFO;
		if (!virtio_submit_sync(kvc, cur_buf_slot, sizeof(*get_info), sizeof(struct virtio_gpu_resp_display_info))) {
			serial_print("[VirtIO:GPU] Failed to submit display command! [Cannot Revert Changes]\n");
			virtio_destroy(kvc);
			return AOS_FALSE;
		}
		struct virtio_gpu_resp_display_info* display = (struct virtio_gpu_resp_display_info*)&kvc->resp_buf[cur_buf_slot];

		if (display->displays[0].enabled) {
			gpu->framebuffer->w = display->displays[0].r.width;
			gpu->framebuffer->h = display->displays[0].r.height;
		} else {
			// Fallback if the device doesn't have a preference
			gpu->framebuffer->w = 1024;
			gpu->framebuffer->h = 768;
		}
		gpu->framebuffer->bpp = 32;
		gpu->framebuffer->pitch = gpu->framebuffer->w * (gpu->framebuffer->bpp / 8);
		if (gpu->framebuffer->pitch * gpu->framebuffer->h != gpu->framebuffer->size) {
			gpu->framebuffer->size = gpu->framebuffer->pitch * gpu->framebuffer->h;
			if (gpu->framebuffer->virt) avmf_free(gpu->framebuffer->virt);
			gpu->framebuffer->virt = avmf_alloc(gpu->framebuffer->size, MALLOC_TYPE_SENSITIVE, AVMF_FLAG_RW | AVMF_FLAG_NO_CACHE, &gpu->framebuffer->phys);
		}

		gpu->active = AOS_TRUE;
		kvc->valid = AOS_TRUE;
	}

	serial_print("[VirtIO:GPU] Refresh Completed!\n");
	return AOS_TRUE;
}

static aos_bool virtio_create_context(virtio_controller* kvc, uint32_t ctx_id, const char* debug_name, size_t debug_name_len) {
    uint64_t cur_buf_slot = wait_for_free_buf(kvc);
    struct virtio_gpu_ctx_create* cmd = (struct virtio_gpu_ctx_create*)&kvc->cmd_buf[cur_buf_slot];
    memset(cmd, 0, sizeof(*cmd));
    
    cmd->hdr.type = VIRTIO_GPU_CMD_CTX_CREATE;
    cmd->hdr.ctx_id = ctx_id;
    cmd->nlen = debug_name_len >= 64 ? 14 : debug_name_len;
    memcpy(cmd->debug_name, debug_name_len >= 64 ? "Pyrion-Context" : debug_name, cmd->nlen);
	cmd->debug_name[cmd->nlen] = '\0';
    
    return virtio_submit_sync(kvc, cur_buf_slot, sizeof(*cmd), sizeof(struct virtio_gpu_ctrl_hdr));
}

static aos_bool virtio_destroy_context(virtio_controller* kvc, uint32_t ctx_id) {
    uint64_t cur_buf_slot = wait_for_free_buf(kvc);
    struct virtio_gpu_ctx_destroy* cmd = (struct virtio_gpu_ctx_destroy*)&kvc->cmd_buf[cur_buf_slot];
    memset(cmd, 0, sizeof(*cmd));
    
    cmd->hdr.type = VIRTIO_GPU_CMD_CTX_DESTROY;
    cmd->hdr.ctx_id = ctx_id;
    
    return virtio_submit_sync(kvc, cur_buf_slot, sizeof(*cmd), sizeof(struct virtio_gpu_ctrl_hdr));
}

// Pyrion implementation
#include <inc/drivers/gpu/apis/pyrion.h>
#define MAX_PYRION_CONTEXTS 256

#define RES_ID_2D_BASE MAX_PYRION_CONTEXTS
#define RES_ID_3D_BASE (MAX_PYRION_CONTEXTS * 2)
#define RES_ID_SCANOUT_BASE (MAX_PYRION_CONTEXTS * 3)
#define RES_ID_RAST_BASE (MAX_PYRION_CONTEXTS * 4)
#define RES_ID_BLEND_BASE (MAX_PYRION_CONTEXTS * 5)

#define RES_ID_FONT_BASE (RES_ID_BLEND_BASE + MAX_PYRION_CONTEXTS)

static struct pyrion_ctx* p_contexts[MAX_PYRION_CONTEXTS];
static uint64_t p_nxt_ctx = 0xFFFFFFFFFFFFFFFF;

aos_bool pyrion_init_virtio(void) {
	uint64_t init = 0;
    for (uint64_t i = 0; i < MAX_PYRION_CONTEXTS; i++) {
        uint64_t ctx_virt = (uint64_t)avmf_alloc(sizeof(struct pyrion_ctx), MALLOC_TYPE_KERNEL, AVMF_FLAG_RW, NULL);
        if (!ctx_virt) continue;

        struct pyrion_ctx* ctx = (struct pyrion_ctx*)ctx_virt;
		memset(ctx, 0, sizeof(*ctx));
        ctx->ctx_id = i + 1;

        ctx->valid = AOS_FALSE; // NOT VALID
		ctx->usable = AOS_FALSE;
		ctx->device_set = AOS_FALSE;
		ctx->viewport_set = AOS_FALSE;

		ctx->flush_needed = AOS_TRUE; // Flush is needed the first time

		ctx->controller_idx = 0; // Default
        p_contexts[i] = ctx;
        if (p_nxt_ctx > MAX_PYRION_CONTEXTS) // Not set yet
            p_nxt_ctx = i;
		init++;
    }
	return init > 0;
}

void pyrion_finish_virtio(void) {
    for (uint64_t i = 0; i < MAX_PYRION_CONTEXTS; i++) {
        struct pyrion_ctx* ctx = (struct pyrion_ctx*)p_contexts[i];
		if (ctx->controller_idx >= controller_count || !controllers) continue;
		virtio_controller* kvc = &controllers[ctx->controller_idx];

        if (!ctx) continue;
        if (ctx->valid) {
            virtio_destroy_context(kvc, ctx->ctx_id);
        }

        avmf_free((uint64_t)ctx);
        p_contexts[i] = NULL;
    }
    p_nxt_ctx = 0xFFFFFFFFFFFFFFFF;
}

static uint64_t pyrion_get_free_ctx_slot(void) {
    if (p_nxt_ctx < MAX_PYRION_CONTEXTS) {
        uint64_t id = p_nxt_ctx;
        p_nxt_ctx = 0xFFFFFFFFFFFFFFFF;
        return id;
    }
    for (uint64_t i = 0; i < MAX_PYRION_CONTEXTS; i++) {
        if (p_contexts[i] && !p_contexts[i]->valid)
            return i;
    }
    return 0xFFFFFFFFFFFFFFFF;
}

static void pyrion_push_virgl_ex(struct pyrion_ctx* ctx, struct pyrion_cmd_stream* stream, uint32_t opcode, uint32_t obj_type, uint32_t* args, uint32_t arg_count) {
	if (!ctx || !stream) return;
	if (!ctx->valid || !stream->stream || !stream->stream_phys) return;
	if (stream->stream_cap < 1 || stream->stream_cap < stream->stream_count) return;

    uint32_t current_size = stream->stream_count;
	if (current_size + ((arg_count + 1)*sizeof(uint32_t)) > stream->stream_cap) return;

    uint32_t* write_ptr = (uint32_t*)((uintptr_t)stream->stream + current_size);
    *write_ptr = VIRGL_CMD_HEADER(opcode, obj_type, arg_count & 0xFF);
    write_ptr++;

    for (uint32_t i = 0; i < arg_count; i++) {
        *write_ptr = args[i];
        write_ptr++;
    }
    
    stream->stream_count += (1 + arg_count) * sizeof(uint32_t);
}

static void pyrion_push_virgl(struct pyrion_ctx* ctx, uint32_t opcode, uint32_t obj_type, uint32_t* args, uint32_t arg_count) {
    if (!ctx) return;
	if (!ctx->valid) return;
	
	pyrion_push_virgl_ex(ctx, &ctx->cmd_stream, opcode, obj_type, args, arg_count);
	ctx->flush_needed = AOS_TRUE;
}

static uint32_t rgba_to_u32(uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
    return ((uint32_t)a << 24) | ((uint32_t)b << 16) | ((uint32_t)g << 8) | ((uint32_t)r);
}

struct pyrion_ctx* pyrion_create_ctx_virtio(struct pyrion_create_ctx_info ctx_info) {
	if (0 >= controller_count || !controllers) return NULL;
	virtio_controller* kvc = &controllers[0];

    serial_print("[Pyrion] Creating Context...\n");

    uint64_t slot = pyrion_get_free_ctx_slot();
    if (slot > MAX_PYRION_CONTEXTS) {
        serial_print("[Pyrion] Context Limit Reached!\n");
        return NULL;
    }

    struct pyrion_ctx* ctx = (struct pyrion_ctx*)p_contexts[slot];
	ctx->cmd_stream.stream_cap = 0x1000;
    ctx->cmd_stream.stream = (void*)avmf_alloc(ctx->cmd_stream.stream_cap, MALLOC_TYPE_DRIVER, AVMF_FLAG_RW | AVMF_FLAG_NO_CACHE, &ctx->cmd_stream.stream_phys);
    if (!ctx->cmd_stream.stream) return NULL;
    memset(ctx->cmd_stream.stream, 0, ctx->cmd_stream.stream_cap);

	ctx_info.name[63] = '\0';
    virtio_create_context(kvc, ctx->ctx_id, ctx_info.name, strlen(ctx_info.name));
    
    ctx->viewport.x = 0; ctx->viewport.y=0; ctx->viewport.width=0; ctx->viewport.height=0; ctx->viewport.color = 0;
    ctx->valid = AOS_TRUE;
	ctx->usable = AOS_FALSE;

	ctx->flush_needed = AOS_TRUE;

    serial_print("[Pyrion] Context Created\n");
    return ctx;
}

aos_bool pyrion_enumerate_physical_devices_virtio(size_t* count, size_t idx, struct pyrion_physical_device* out) {
	if (!out) {
		if (!count) return AOS_FALSE;
		*count = controller_count;
		return AOS_TRUE;
	}
	
	if (idx >= controller_count) return AOS_FALSE;
	virtio_controller* kvc = &controllers[idx];

	if (!kvc->valid) return AOS_FALSE;
	memcpy(out->name, "Virtio-GPU", 11);

	out->cmd_core = kvc->gpu_cmd_core;
	out->accelerated = kvc->acceleration_present;
	out->idx = kvc->idx;

	return AOS_TRUE;
}

static aos_bool pyrion_create_viewport_res(struct pyrion_ctx* ctx, virtio_controller* kvc, struct pyrion_rect viewport) {
	if (!ctx || !kvc) return AOS_FALSE;

	// Resource Initialize
	ctx->res_id_3d = ctx->ctx_id + RES_ID_3D_BASE; // Ensure not a single resource id causes trouble
	ctx->res_id_2d = ctx->ctx_id + RES_ID_2D_BASE;

	// Create 2D Texture
	uint64_t slot = wait_for_free_buf(kvc);

	struct virtio_gpu_resource_create_2d* c2d = (struct virtio_gpu_resource_create_2d*)&kvc->cmd_buf[slot];
	memset(c2d, 0, sizeof(*c2d));
	c2d->hdr.type = VIRTIO_GPU_CMD_RESOURCE_CREATE_2D;
	c2d->resource_id = ctx->res_id_2d;
	c2d->format = VIRTIO_GPU_FORMAT_R8G8B8A8_UNORM;
	c2d->width = viewport.width;
	c2d->height = viewport.height;

	if (!virtio_submit_sync(kvc, slot, sizeof(*c2d), sizeof(struct virtio_gpu_ctrl_hdr))) return AOS_FALSE;

	slot = wait_for_free_buf(kvc);

	struct virtio_gpu_ctx_resource* att = (struct virtio_gpu_ctx_resource*)&kvc->cmd_buf[slot];
	memset(att, 0, sizeof(*att));
	att->hdr.type = VIRTIO_GPU_CMD_CTX_ATTACH_RESOURCE;
	att->hdr.ctx_id = ctx->ctx_id;
	att->resource_id = ctx->res_id_2d;
	
	if (!virtio_submit_sync(kvc, slot, sizeof(*att), sizeof(struct virtio_gpu_ctrl_hdr))) return AOS_FALSE;

	if (kvc->acceleration_present) {
		// Create 3D-Accelerated Texture
		slot = wait_for_free_buf(kvc);

		struct virtio_gpu_resource_create_3d* c3d = (struct virtio_gpu_resource_create_3d*)&kvc->cmd_buf[slot];
		memset(c3d, 0, sizeof(*c3d));
		c3d->hdr.type = VIRTIO_GPU_CMD_RESOURCE_CREATE_3D;
		c3d->resource_id = ctx->res_id_3d;
		c3d->target = PIPE_TEXTURE_2D;
		c3d->format = PIPE_FORMAT_R8G8B8A8_UNORM;
		c3d->bind = PIPE_BIND_RENDER_TARGET | PIPE_BIND_SAMPLER_VIEW;
		c3d->width = viewport.width;
		c3d->height = viewport.height;
		c3d->depth = 1;
		c3d->last_level = 0;
		c3d->nr_samples = 1;
		c3d->array_size = 1;
		c3d->flags = VIRTIO_GPU_RESOURCE_FLAG_Y_0_TOP;

		if (!virtio_submit_sync(kvc, slot, sizeof(*c3d), sizeof(struct virtio_gpu_ctrl_hdr))) return AOS_FALSE;

		slot = wait_for_free_buf(kvc);

		struct virtio_gpu_ctx_resource* att = (struct virtio_gpu_ctx_resource*)&kvc->cmd_buf[slot];
		memset(att, 0, sizeof(*att));
		att->hdr.type = VIRTIO_GPU_CMD_CTX_ATTACH_RESOURCE;
		att->hdr.ctx_id = ctx->ctx_id;
		att->resource_id = ctx->res_id_3d;
		
		if (!virtio_submit_sync(kvc, slot, sizeof(*att), sizeof(struct virtio_gpu_ctrl_hdr))) return AOS_FALSE;
	}

	// Create FB
	ctx->fb_info.cformat = PYRION_COLORF_RGBA;
	ctx->fb_info.width = viewport.width;
	ctx->fb_info.height = viewport.height;
	ctx->fb_info.bpp = sizeof(uint32_t) * 8;
	ctx->fb_info.pitch = viewport.width * sizeof(uint32_t);
	ctx->fb_info.size = ctx->fb_info.width * ctx->fb_info.height * sizeof(uint32_t);
	ctx->fb_info.addr = avmf_alloc(ctx->fb_info.size, MALLOC_TYPE_SENSITIVE, AVMF_FLAG_RW | AVMF_FLAG_NO_CACHE | AVMF_FLAG_GLOBAL, &ctx->fb_info.phys_addr);
	
	if (!ctx->fb_info.addr || !ctx->fb_info.phys_addr) return AOS_FALSE;
	memset((void*)ctx->fb_info.addr, 0, ctx->fb_info.size);

	// Attach Backing
	slot = wait_for_free_buf(kvc);

    struct attb {
        struct virtio_gpu_resource_attach_backing att;
        struct virtio_gpu_mem_entry entry;
    } __attribute__((packed));

    struct attb* attb = (struct attb*)&kvc->cmd_buf[slot];
    memset(attb, 0, sizeof(*attb));
    attb->att.hdr.type = VIRTIO_GPU_CMD_RESOURCE_ATTACH_BACKING;
    attb->att.resource_id = ctx->res_id_2d;
    attb->att.nr_entries = 1;
    attb->entry.addr = (uint64_t)ctx->fb_info.phys_addr;
    attb->entry.length = ctx->fb_info.size;

    if (!virtio_submit_sync(kvc, slot, sizeof(*attb), sizeof(struct virtio_gpu_ctrl_hdr))) {
		avmf_free(ctx->fb_info.addr);
		ctx->fb_info.addr = 0;
		ctx->fb_info.phys_addr = 0;
		return AOS_FALSE;
	}

	return AOS_TRUE;
}

static void pyrion_delete_viewport_res(struct pyrion_ctx* ctx, virtio_controller* kvc) {
	if (!ctx || !kvc) return;

	uint64_t slot;
	if (kvc->acceleration_present && ctx->device.accelerated) {
		if (ctx->res_id_3d > RES_ID_3D_BASE && ctx->res_id_3d <= RES_ID_3D_BASE + MAX_PYRION_CONTEXTS) {
			// Detach 3D Resource
			slot = wait_for_free_buf(kvc);

			struct virtio_gpu_ctx_resource* d = (struct virtio_gpu_ctx_resource*)&kvc->cmd_buf[slot];
			memset(d, 0, sizeof(*d));

			d->hdr.type = VIRTIO_GPU_CMD_CTX_DETACH_RESOURCE;
			d->hdr.ctx_id = ctx->ctx_id;
			d->resource_id = ctx->res_id_3d;

			virtio_submit_sync(kvc, slot, sizeof(*d), sizeof(struct virtio_gpu_ctrl_hdr));

			// Destroy 3D Resource
			slot = wait_for_free_buf(kvc);
			struct virtio_gpu_resource_unref* u = (struct virtio_gpu_resource_unref*)&kvc->cmd_buf[slot];
			memset(u, 0, sizeof(*u));

			u->hdr.type = VIRTIO_GPU_CMD_RESOURCE_UNREF;
        	u->resource_id = ctx->res_id_3d;

			virtio_submit_sync(kvc, slot, sizeof(*u), sizeof(struct virtio_gpu_ctrl_hdr));
		}
	}
	ctx->res_id_3d = 0;

	if (ctx->res_id_2d > RES_ID_2D_BASE && ctx->res_id_2d <= RES_ID_2D_BASE + MAX_PYRION_CONTEXTS) {
		// Detach Backing for 2D Resource
		slot = wait_for_free_buf(kvc);

		struct virtio_gpu_resource_detach_backing* d = (struct virtio_gpu_resource_detach_backing*)&kvc->cmd_buf[slot];
		memset(d, 0, sizeof(*d));

		d->hdr.type = VIRTIO_GPU_CMD_RESOURCE_DETACH_BACKING;
		d->resource_id = ctx->res_id_2d;

		virtio_submit_sync(kvc, slot, sizeof(*d), sizeof(struct virtio_gpu_ctrl_hdr));

		// Destroy 2D Resource
		slot = wait_for_free_buf(kvc);
		struct virtio_gpu_resource_unref* u = (struct virtio_gpu_resource_unref*)&kvc->cmd_buf[slot];
		memset(u, 0, sizeof(*u));

		u->hdr.type = VIRTIO_GPU_CMD_RESOURCE_UNREF;
		u->resource_id = ctx->res_id_2d;

		virtio_submit_sync(kvc, slot, sizeof(*u), sizeof(struct virtio_gpu_ctrl_hdr));
	}
	ctx->res_id_2d = 0;

	if (ctx->fb_info.addr) {
        avmf_free(ctx->fb_info.addr);
        ctx->fb_info.addr = 0;
        ctx->fb_info.phys_addr = 0;
    }
}

aos_bool pyrion_use_device_virtio(struct pyrion_ctx* ctx, struct pyrion_physical_device* dev) {
	if (!dev || !ctx) return AOS_FALSE;
	if (ctx->usable || !ctx->valid || dev->idx >= controller_count || ctx->device_set) return AOS_FALSE;
	if (!controllers[dev->idx].valid) return AOS_FALSE;

	// Device Verifications
	virtio_controller* kvc = &controllers[dev->idx];
	if (kvc->acceleration_present != dev->accelerated) return AOS_FALSE;
	if (kvc->gpu_cmd_core != dev->cmd_core) return AOS_FALSE;
	if (kvc->idx != dev->idx) return AOS_FALSE;

	// Device Registering
	struct pyrion_physical_device dev_cp = *dev;
	ctx->device = dev_cp;
	ctx->controller_idx = dev->idx;
	ctx->device_set = AOS_TRUE;

	// Set Rasterizer
	ctx->res_id_rast = ctx->ctx_id + RES_ID_RAST_BASE;
	uint32_t rast_handle = ctx->res_id_rast;
    uint32_t rast_args[VIRGL_OBJ_RS_SIZE] = {0};
    rast_args[VIRGL_OBJ_RS_HANDLE] = rast_handle;
    rast_args[VIRGL_OBJ_RS_S0] = VIRGL_OBJ_RS_S0_FILL_FRONT(2) | VIRGL_OBJ_RS_S0_FILL_BACK(2) | VIRGL_OBJ_RS_S0_FRONT_CCW(1) | VIRGL_OBJ_RS_S0_FLATSHADE(1);
	rast_args[VIRGL_OBJ_RS_POINT_SIZE] = 1;
	rast_args[VIRGL_OBJ_RS_LINE_WIDTH] = 1;
    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_CREATE_OBJECT, VIRTIO_VIRGL_OBJECT_RASTERIZER, (uint32_t*)rast_args, VIRGL_OBJ_RS_SIZE);
    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_BIND_OBJECT, VIRTIO_VIRGL_OBJECT_RASTERIZER, &rast_handle, 1);

	// Set Blend State
	ctx->res_id_blend = ctx->ctx_id + RES_ID_BLEND_BASE;
	uint32_t blend_handle = ctx->res_id_blend;
    uint32_t blend_args[VIRGL_OBJ_BLEND_SIZE] = {0};
    blend_args[VIRGL_OBJ_BLEND_HANDLE] = blend_handle;
    blend_args[VIRGL_OBJ_BLEND_S1] = VIRGL_OBJ_BLEND_S1_LOGICOP_FUNC(PIPE_LOGICOP_NOOP);
	blend_args[VIRGL_OBJ_BLEND_S2(0)] = VIRGL_OBJ_BLEND_S2_RT_RGB_SRC_FACTOR(PIPE_BLENDFACTOR_ONE) | VIRGL_OBJ_BLEND_S2_RT_RGB_FUNC(PIPE_BLEND_ADD) | VIRGL_OBJ_BLEND_S2_RT_ALPHA_SRC_FACTOR(PIPE_BLENDFACTOR_ONE) | VIRGL_OBJ_BLEND_S2_RT_ALPHA_FUNC(PIPE_BLEND_ADD);
    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_CREATE_OBJECT, VIRTIO_VIRGL_OBJECT_BLEND, (uint32_t*)blend_args, VIRGL_OBJ_BLEND_SIZE);
    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_BIND_OBJECT, VIRTIO_VIRGL_OBJECT_BLEND, &blend_handle, 1);

	// Flush VIRGL
	if (!pyrion_submit_cmd_stream_virtio(ctx, &ctx->cmd_stream)) {
		pyrion_unuse_device(ctx);
		return AOS_FALSE;
	}
	return AOS_TRUE;
}

void pyrion_unuse_device_virtio(struct pyrion_ctx* ctx) {
	if (!ctx) return;
	if (!ctx->device_set || !ctx->valid) return;
	if (ctx->controller_idx >= controller_count || !controllers) return;

	ctx->device_set = AOS_FALSE;
	ctx->usable = AOS_FALSE;

	if (ctx->res_id_rast > RES_ID_RAST_BASE && ctx->res_id_rast <= RES_ID_RAST_BASE + MAX_PYRION_CONTEXTS) {
		uint32_t args[1] = {ctx->res_id_rast};
		pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_DESTROY_OBJECT, VIRTIO_VIRGL_OBJECT_RASTERIZER, (uint32_t*)args, 1);
	}
	ctx->res_id_rast = 0;

	if (ctx->res_id_blend > RES_ID_BLEND_BASE && ctx->res_id_blend <= RES_ID_BLEND_BASE + MAX_PYRION_CONTEXTS) {
		uint32_t args[1] = {ctx->res_id_blend};
		pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_DESTROY_OBJECT, VIRTIO_VIRGL_OBJECT_BLEND, (uint32_t*)args, 1);
	}
	ctx->res_id_blend = 0;

	pyrion_submit_cmd_stream_virtio(ctx, &ctx->cmd_stream);
}

void pyrion_destroy_ctx_virtio(struct pyrion_ctx* ctx) {
    if (!ctx) return;
	if (!ctx->valid) return;
	if (ctx->controller_idx >= controller_count || !controllers) return;
	virtio_controller* kvc = &controllers[ctx->controller_idx];

	uint64_t slot = wait_for_free_buf(kvc);
	
	struct virtio_gpu_set_scanout* scanout = (struct virtio_gpu_set_scanout*)&kvc->cmd_buf[slot];
    memset(scanout, 0, sizeof(*scanout));
    scanout->hdr.type = VIRTIO_GPU_CMD_SET_SCANOUT;
    scanout->resource_id = 0; // Reset
    scanout->scanout_id = 0; // The first monitor
    scanout->r.width = 0;
    scanout->r.height = 0;
    scanout->r.x = 0;
    scanout->r.y = 0;

	virtio_submit_sync(kvc, slot, sizeof(*scanout), sizeof(struct virtio_gpu_ctrl_hdr));
	pyrion_delete_viewport_res(ctx, kvc);

	if (ctx->device_set) pyrion_unuse_device_virtio(ctx);
	ctx->device_set = AOS_FALSE;
	ctx->usable = AOS_FALSE;
	ctx->valid = AOS_FALSE;

    virtio_destroy_context(kvc, ctx->ctx_id);

    if (ctx->cmd_stream.stream) {
		avmf_free((uint64_t)ctx->cmd_stream.stream);
		ctx->cmd_stream.stream = NULL;
		ctx->cmd_stream.stream_phys = 0;
		ctx->cmd_stream.stream_cap = 0;
		ctx->cmd_stream.stream_count = 0;
	}

	p_nxt_ctx = ctx->ctx_id;
	memset(ctx, 0, sizeof(struct pyrion_ctx));
}

aos_bool pyrion_submit_cmd_stream_virtio(struct pyrion_ctx* ctx, struct pyrion_cmd_stream* stream) {
	if (!ctx || !stream) return AOS_FALSE;
	if (!ctx->valid || !stream->stream || !stream->stream_phys) return AOS_FALSE;
	if (ctx->controller_idx >= controller_count || !controllers) return AOS_FALSE;

	if (stream->stream_cap < 1 || stream->stream_cap < stream->stream_count) return AOS_FALSE;
	if (stream->stream_count < 1) return AOS_TRUE;

	virtio_controller* kvc = &controllers[ctx->controller_idx];

	for (uintptr_t i = (uintptr_t)stream->stream; i < (uintptr_t)stream->stream + stream->stream_count; i += 64) { // 64 is the standard x86 cache line size
        __asm__ volatile("clflush (%0)" : : "r"(i) : "memory");
    }
    __asm__ volatile("mfence" ::: "memory");

	uint64_t slot = wait_for_free_buf(kvc);

    struct virtio_gpu_cmd_submit_3d* virgl = (struct virtio_gpu_cmd_submit_3d*)&kvc->cmd_buf[slot];
	memset(virgl, 0, sizeof(*virgl));
    virgl->hdr.type = VIRTIO_GPU_CMD_SUBMIT_3D;
    virgl->hdr.ctx_id = ctx->ctx_id;
    virgl->size = stream->stream_count;

	struct virtq_desc descs[3] = {
		(struct virtq_desc){
			.addr = (uintptr_t)kvc->cmd_buf_phys[slot],
			.len = sizeof(*virgl),
			.flags = VIRTQ_DESC_F_NEXT,
			.next = 0 // Autofill
		},
		(struct virtq_desc){
			.addr = (uintptr_t)(stream->stream_phys),
			.len = stream->stream_count,
			.flags = VIRTQ_DESC_F_NEXT,
			.next = 0 // Autofill
		},
		(struct virtq_desc){
			.addr = (uintptr_t)(kvc->resp_buf_phys[slot]),
			.len = sizeof(struct virtio_gpu_ctrl_hdr),
			.flags = VIRTQ_DESC_F_WRITE,
			.next = 0xFFFF
		}
	};

	uint16_t head = 0;
	if (!virtio_send_ex(kvc, (struct virtq_desc*)descs, 3, &head)) return AOS_FALSE;
	kvc->desc_to_buf[head] = slot;
	virtio_notify_device(kvc);
	aos_bool out = virtqueue_wait_complete_sync(kvc);

	if (!out) return out;
	stream->stream_count = 0;
	return out;
}

aos_bool pyrion_viewport_virtio(struct pyrion_ctx* ctx, struct pyrion_rect viewport) {
    if (!ctx) return AOS_FALSE;
	if (!ctx->device_set || !ctx->valid) return AOS_FALSE;
	if (ctx->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	virtio_controller* kvc = &controllers[ctx->controller_idx];

	if (ctx->viewport_set) {
		if (ctx->usable) {
			if (!pyrion_submit_cmd_stream_virtio(ctx, &ctx->cmd_stream)) return AOS_FALSE;
			ctx->usable = AOS_FALSE;
		}

		if (viewport.width != ctx->viewport.width || viewport.height != ctx->viewport.height) {
			ctx->viewport_set = AOS_FALSE;
			pyrion_delete_viewport_res(ctx, kvc);
			if (!pyrion_create_viewport_res(ctx, kvc, viewport)) return AOS_FALSE;
		}

		uint32_t surf_args[1] = {ctx->res_id_scanout};
		pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_DESTROY_OBJECT, VIRTIO_VIRGL_OBJECT_SURFACE, (uint32_t*)surf_args, 1);
	} else {
		if (!pyrion_create_viewport_res(ctx, kvc, viewport)) return AOS_FALSE;
	}

	ctx->viewport = viewport;
	ctx->res_id_scanout = ctx->ctx_id + RES_ID_SCANOUT_BASE;

	// Set scanout
    uint64_t slot = wait_for_free_buf(kvc);

    struct virtio_gpu_set_scanout* scanout = (struct virtio_gpu_set_scanout*)&kvc->cmd_buf[slot];
    memset(scanout, 0, sizeof(*scanout));
    scanout->hdr.type = VIRTIO_GPU_CMD_SET_SCANOUT;
    scanout->resource_id = ctx->res_id_2d;
    scanout->scanout_id = 0; // The first monitor
    scanout->r.width = viewport.width;
    scanout->r.height = viewport.height;
    scanout->r.x = 0;
    scanout->r.y = 0;

    if (!virtio_submit_sync(kvc, slot, sizeof(*scanout), sizeof(struct virtio_gpu_ctrl_hdr))) {
		return AOS_FALSE;
	}
	
	if (!kvc->acceleration_present) {
        serial_print("[VirtIO:GPU] No Acceleration in Viewport/Context!\n");
		ctx->usable = AOS_TRUE;
		ctx->viewport_set = AOS_TRUE;
		return AOS_TRUE;
    }

    uint32_t surf_args[VIRGL_OBJ_SURFACE_SIZE] = {0};
	surf_args[VIRGL_OBJ_SURFACE_HANDLE] = ctx->res_id_scanout;
	surf_args[VIRGL_OBJ_SURFACE_RES_HANDLE] = ctx->res_id_3d;
	surf_args[VIRGL_OBJ_SURFACE_FORMAT] = PIPE_FORMAT_R8G8B8A8_UNORM;
    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_CREATE_OBJECT, VIRTIO_VIRGL_OBJECT_SURFACE, (uint32_t*)surf_args, VIRGL_OBJ_SURFACE_SIZE);
    
	uint32_t fb_args[VIRGL_SET_FRAMEBUFFER_STATE_SIZE(1)] = {0};
	fb_args[VIRGL_SET_FRAMEBUFFER_STATE_NR_CBUFS] = 1;
	fb_args[VIRGL_SET_FRAMEBUFFER_STATE_CBUF_HANDLE(0)] = ctx->res_id_scanout;
    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_SET_FRAMEBUFFER_STATE, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)fb_args, VIRGL_SET_FRAMEBUFFER_STATE_SIZE(1));

    float width = (float)viewport.width;
    float height = (float)viewport.height;

    uint32_t view_args[VIRGL_SET_VIEWPORT_STATE_SIZE(1)] = {0};
    ((float*)view_args)[VIRGL_SET_VIEWPORT_STATE_SCALE_0(0)] = width / 2.0f; // Scale X
    ((float*)view_args)[VIRGL_SET_VIEWPORT_STATE_SCALE_1(0)] = height / 2.0f; // Scale Y
    ((float*)view_args)[VIRGL_SET_VIEWPORT_STATE_SCALE_2(0)] = 0.5f; // Scale Z (Depth)
    ((float*)view_args)[VIRGL_SET_VIEWPORT_STATE_TRANSLATE_0(0)] = width / 2.0f; // Translate X
    ((float*)view_args)[VIRGL_SET_VIEWPORT_STATE_TRANSLATE_1(0)] = height / 2.0f; // Translate Y
    ((float*)view_args)[VIRGL_SET_VIEWPORT_STATE_TRANSLATE_2(0)] = 0.5f; // Translate Z
    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_SET_VIEWPORT_STATE, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)view_args, VIRGL_SET_VIEWPORT_STATE_SIZE(1));

    uint32_t init_scissor[VIRGL_SET_SCISSOR_STATE_SIZE(1)] = {0};
	init_scissor[VIRGL_SET_SCISSOR_MINX_MINY(0)] = (0 << 16) | 0;
	init_scissor[VIRGL_SET_SCISSOR_MAXX_MAXY(0)] = ((viewport.height & 0xFFFF) << 16) | (viewport.width & 0xFFFF);
    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_SET_SCISSOR_STATE, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)init_scissor, VIRGL_SET_SCISSOR_STATE_SIZE(1));

	ctx->usable = AOS_TRUE;
	ctx->viewport_set = AOS_TRUE;

    if (!pyrion_flush_virtio(ctx)) {
		return AOS_FALSE;
	}
	return AOS_TRUE;
}

aos_bool pyrion_flush_virtio(struct pyrion_ctx* ctx) {
    if (!ctx) return AOS_FALSE;
	if (!ctx->valid || !ctx->usable || !ctx->device_set || !ctx->viewport_set) return AOS_FALSE;
	if (ctx->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	
	if (!ctx->flush_needed) return AOS_TRUE;
	virtio_controller* kvc = &controllers[ctx->controller_idx];

    if (!kvc->acceleration_present) {
		uintptr_t start = (uintptr_t)ctx->fb_info.addr;
        uintptr_t end = start + (ctx->fb_info.width * ctx->fb_info.height * sizeof(uint32_t));
        for (uintptr_t i = start; i < end; i += 64) {
            __asm__ volatile("clflush (%0)" : : "r"(i) : "memory");
        }
        __asm__ volatile("mfence" ::: "memory");

		uint64_t slot = wait_for_free_buf(kvc);
        struct virtio_gpu_transfer_to_host_2d* t2d = (struct virtio_gpu_transfer_to_host_2d*)&kvc->cmd_buf[slot];
        memset(t2d, 0, sizeof(*t2d));
        t2d->hdr.type = VIRTIO_GPU_CMD_TRANSFER_TO_HOST_2D;
		t2d->hdr.ctx_id = ctx->ctx_id;
        t2d->resource_id = ctx->res_id_2d;
        t2d->r.x = ctx->viewport.x;
        t2d->r.y = ctx->viewport.y;
        t2d->r.width = ctx->viewport.width;
        t2d->r.height = ctx->viewport.height;
        t2d->offset = 0;

        if (!virtio_submit_sync(kvc, slot, sizeof(*t2d), sizeof(struct virtio_gpu_ctrl_hdr))) return AOS_FALSE;

        virtio_flush(kvc->gpu, ctx->viewport.x, ctx->viewport.y, ctx->viewport.width, ctx->viewport.height, ctx->res_id_2d);

		ctx->flush_needed = AOS_FALSE;
        return AOS_TRUE;
    }

	if (!pyrion_blit_virtio(ctx, ctx->res_id_2d, ctx->res_id_3d, ctx->viewport.width, ctx->viewport.height)) return AOS_FALSE;
    if (!pyrion_submit_cmd_stream_virtio(ctx, &ctx->cmd_stream)) return AOS_FALSE;

    uint64_t slot = wait_for_free_buf(kvc);
    struct virtio_gpu_transfer_to_host_2d* t = (struct virtio_gpu_transfer_to_host_2d*)&kvc->cmd_buf[slot];
    memset(t, 0, sizeof(*t));
    t->hdr.type = VIRTIO_GPU_CMD_TRANSFER_TO_HOST_2D;
    t->hdr.ctx_id = ctx->ctx_id;
    t->resource_id = ctx->res_id_2d;
    t->r.x = ctx->viewport.x;
	t->r.y = ctx->viewport.y;
	t->r.width = ctx->viewport.width;
	t->r.height = ctx->viewport.height;
	t->offset = 0;

    if (!virtio_submit_sync(kvc, slot, sizeof(*t), sizeof(struct virtio_gpu_ctrl_hdr))) return AOS_FALSE;
    
    slot = wait_for_free_buf(kvc);

    struct virtio_gpu_resource_flush* f = (struct virtio_gpu_resource_flush*)&kvc->cmd_buf[slot];
    memset(f, 0, sizeof(*f));
    f->hdr.type = VIRTIO_GPU_CMD_RESOURCE_FLUSH;
    f->hdr.ctx_id = ctx->ctx_id;
    f->r.x = ctx->viewport.x;
    f->r.y = ctx->viewport.y;
    f->r.width = ctx->viewport.width;
    f->r.height = ctx->viewport.height;
    f->resource_id = ctx->res_id_2d;
    f->padding = 0;

	ctx->flush_needed = AOS_FALSE;
    return virtio_submit_async(kvc, slot, sizeof(*f), sizeof(struct virtio_gpu_ctrl_hdr));
}

aos_bool pyrion_clear_virtio(struct pyrion_ctx* ctx, uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
	if (!ctx) return AOS_FALSE;
	if (!ctx->valid || !ctx->usable || !ctx->device_set) return AOS_FALSE;
	if (ctx->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	virtio_controller* kvc = &controllers[ctx->controller_idx];

    if (!kvc->acceleration_present) {
        if (!ctx->fb_info.addr) return AOS_FALSE;
        fb_clear(&ctx->fb_info, rgba_to_u32(r, g, b, a));
		ctx->flush_needed = AOS_TRUE;
        return AOS_TRUE;
    }

    float color[4] = {r / 255.0f, g / 255.0f, b / 255.0f, a / 255.0f};
    uint32_t args[VIRGL_OBJ_CLEAR_SIZE] = {0};
    args[VIRGL_OBJ_CLEAR_BUFFERS] = PIPE_CLEAR_COLOR;
	memcpy(&args[VIRGL_OBJ_CLEAR_COLOR_0], &color[0], sizeof(float));
    memcpy(&args[VIRGL_OBJ_CLEAR_COLOR_1], &color[1], sizeof(float));
    memcpy(&args[VIRGL_OBJ_CLEAR_COLOR_2], &color[2], sizeof(float));
    memcpy(&args[VIRGL_OBJ_CLEAR_COLOR_3], &color[3], sizeof(float));

    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_CLEAR, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)args, VIRGL_OBJ_CLEAR_SIZE);
	return AOS_TRUE;
}

aos_bool pyrion_pixel_virtio(struct pyrion_ctx* ctx, uint32_t x, uint32_t y, uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
	if (!ctx) return AOS_FALSE;
	if (!ctx->valid || !ctx->usable || !ctx->device_set) return AOS_FALSE;
	if (ctx->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	virtio_controller* kvc = &controllers[ctx->controller_idx];
	
	if (x > ctx->viewport.x + ctx->viewport.width || y > ctx->viewport.y + ctx->viewport.height) return AOS_FALSE; // Out of Bounds

    if (!kvc->acceleration_present) {
        if (!ctx->fb_info.addr) return AOS_FALSE;
        fb_put_pixel(&ctx->fb_info, x, y, rgba_to_u32(r, g, b, a));
		ctx->flush_needed = AOS_TRUE;
        return AOS_TRUE;
    }
    uint32_t scissor_args[VIRGL_SET_SCISSOR_STATE_SIZE(1)] = {0};
    scissor_args[VIRGL_SET_SCISSOR_MINX_MINY(0)] = ((y & 0xFFFF) << 16) | (x & 0xFFFF);
    scissor_args[VIRGL_SET_SCISSOR_MAXX_MAXY(0)] = (((y + 1) & 0xFFFF) << 16) | ((x + 1) & 0xFFFF);

    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_SET_SCISSOR_STATE, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)scissor_args, VIRGL_SET_SCISSOR_STATE_SIZE(1));
    
    float color[4] = {r / 255.0f, g / 255.0f, b / 255.0f, a / 255.0f};
    uint32_t args[VIRGL_OBJ_CLEAR_SIZE] = {0};
    args[VIRGL_OBJ_CLEAR_BUFFERS] = PIPE_CLEAR_COLOR;
	((float*)args)[VIRGL_OBJ_CLEAR_COLOR_0] = color[0];
	((float*)args)[VIRGL_OBJ_CLEAR_COLOR_1] = color[1];
	((float*)args)[VIRGL_OBJ_CLEAR_COLOR_2] = color[2];
	((float*)args)[VIRGL_OBJ_CLEAR_COLOR_3] = color[3];

    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_CLEAR, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)args, VIRGL_OBJ_CLEAR_SIZE);

    scissor_args[VIRGL_SET_SCISSOR_MINX_MINY(0)] = ((ctx->viewport.y & 0xFFFF) << 16) | (ctx->viewport.x & 0xFFFF);
    scissor_args[VIRGL_SET_SCISSOR_MAXX_MAXY(0)] = (((ctx->viewport.y + ctx->viewport.height) & 0xFFFF) << 16) | ((ctx->viewport.x + ctx->viewport.width) & 0xFFFF);

    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_SET_SCISSOR_STATE, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)scissor_args, VIRGL_SET_SCISSOR_STATE_SIZE(1));
	return AOS_TRUE;
}

aos_bool pyrion_draw_char_virtio(struct pyrion_ctx* ctx, uint32_t x, uint32_t y, uint32_t codepoint, struct pyrion_font* font, struct pyrion_glyph* glyph) {
	if (!ctx || !font || !glyph) return AOS_FALSE;
	if (!ctx->valid || !ctx->usable || !ctx->device_set) return AOS_FALSE;
	if (!font->valid) return AOS_FALSE;
	if (!glyph->valid || glyph->codepoint != codepoint) return AOS_FALSE;

	if (ctx->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	virtio_controller* kvc = &controllers[ctx->controller_idx];

	float scale = (float)font->font_size / (float)font->base_font_size;
	if (scale <= 0.0f) {
		serial_printf("[VirtIO:GPU] Calculated Font Scale <= 0 (Char U+%04lX)\n", codepoint);
		return AOS_FALSE;
	}

	int64_t bearing_x = (int64_t)kround(glyph->bearing_x * scale);
	int64_t bearing_y = (int64_t)kround(glyph->bearing_y * scale);

	int64_t draw_x_i = (int64_t)x + bearing_x;
	int64_t draw_y_i = (int64_t)y - bearing_y;

	if (draw_x_i < 0) {
		serial_printf("[VirtIO:GPU] Char U+%04lX Has Invalid Bearing X! (%d) (Calculated Final Value: %d)\n", codepoint, glyph->bearing_x, draw_x_i);
		return AOS_FALSE;
	}
	if (draw_y_i < 0) {
		serial_printf("[VirtIO:GPU] Char U+%04lX Has Invalid Bearing Y! (%d) (Calculated Final Value: %d)\n", codepoint, glyph->bearing_y, draw_y_i);
		return AOS_FALSE;
	}

	uint32_t draw_width = kmax((uint64_t)((uint32_t)kceil(glyph->width * scale)), 1);
	uint32_t draw_height = kmax((uint64_t)((uint32_t)kceil(glyph->height * scale)), 1);

	uint32_t draw_x = (uint32_t)draw_x_i;
	uint32_t draw_y = (uint32_t)draw_y_i;
	
	if (
		draw_x < ctx->viewport.x ||
		draw_y < ctx->viewport.y ||
		draw_x + draw_width > ctx->viewport.x + ctx->viewport.width ||
		draw_y + draw_height > ctx->viewport.y + ctx->viewport.height
	) {
		serial_printf("[VirtIO:GPU] Cannot render outside viewport! (Char U+%04lX) (Draw-(%u, %u) Scale-(%u, %u) Viewport-(%u-%u, %u-%u)\n", codepoint, draw_x, draw_y, draw_width, draw_height, ctx->viewport.x, ctx->viewport.x + ctx->viewport.width, ctx->viewport.y, ctx->viewport.y + ctx->viewport.height);
		return AOS_FALSE; // Out of Bounds
	}

    if (!kvc->acceleration_present) {
        if (!ctx->fb_info.addr) {
			serial_printf("[VirtIO:GPU] No Frambuffer Address? (Char U+%04lX)\n", codepoint);
			return AOS_FALSE;
		}
		if (!font->atlas) {
			serial_printf("[VirtIO:GPU] No Font Atlas? (Char U+%04lX)\n", codepoint);
			return AOS_FALSE;
		}

		for (uint64_t dy = 0; dy < draw_height; dy++) {
			uint32_t src_y = (uint32_t)(dy / scale);
			if (src_y >= glyph->height) src_y = glyph->height - 1;
			uint32_t* src = (uint32_t*)((uintptr_t)font->atlas + (((glyph->atlas_y + src_y) * font->w) + glyph->atlas_x) * sizeof(uint32_t));

			for (uint32_t dx = 0; dx < draw_width; dx++) {
				uint32_t src_x = (uint32_t)(dx / scale);
				if (src_x >= glyph->width) src_x = glyph->width - 1;
				uint32_t src_pixel = src[src_x];

				uint8_t sr = src_pixel & 0xFF;
				uint8_t sg = (src_pixel >> 8) & 0xFF;
				uint8_t sb = (src_pixel >> 16) & 0xFF;
				uint8_t sa = (src_pixel >> 24) & 0xFF;

				if (sa == 0) continue;

				uint32_t* dst = (uint32_t*)((uintptr_t)ctx->fb_info.addr + (((draw_y + dy) * ctx->fb_info.width) + draw_x + dx) * (ctx->fb_info.bpp / 8));
				uint32_t dst_pixel = dst[0];

				uint8_t dr = dst_pixel & 0xFF;
				uint8_t dg = (dst_pixel >> 8) & 0xFF;
				uint8_t db = (dst_pixel >> 16) & 0xFF;
				uint8_t da = (dst_pixel >> 24) & 0xFF;

				if (sa == 0xFF) {
					dst[0] = rgba_to_u32(sr, sg, sb, 0xFF);
					continue;
				}

				uint8_t or = (uint8_t)(((uint32_t)sr * sa + (uint32_t)dr * (0xFF - sa) + 0x7F) / 0xFF);
				uint8_t og = (uint8_t)(((uint32_t)sg * sa + (uint32_t)dg * (0xFF - sa) + 0x7F) / 0xFF);
				uint8_t ob = (uint8_t)(((uint32_t)sb * sa + (uint32_t)db * (0xFF - sa) + 0x7F) / 0xFF);
				uint8_t oa = (uint8_t)(sa + ((uint32_t)da * (0xFF - sa) + 0x7F) / 0xFF);

				dst[0] = rgba_to_u32(or, og, ob, oa);
			}
		}
        
		ctx->flush_needed = AOS_TRUE;
        return AOS_TRUE;
    }
    
	// TODO: Add Scaling in VirGL path
	uint32_t args[VIRGL_CMD_RESOURCE_COPY_REGION_SIZE] = {0};
    args[VIRGL_CMD_RCR_DST_RES_HANDLE] = ctx->res_id_3d; // Destination (The Window)
    args[VIRGL_CMD_RCR_DST_LEVEL] = 0; // Dest level
    args[VIRGL_CMD_RCR_DST_X] = draw_x; // Dest X
    args[VIRGL_CMD_RCR_DST_Y] = draw_y; // Dest Y
    args[VIRGL_CMD_RCR_DST_Z] = 0; // Dest Z
    args[VIRGL_CMD_RCR_SRC_RES_HANDLE] = font->res_id; // Source (The Font Atlas)
    args[VIRGL_CMD_RCR_SRC_LEVEL] = 0; // Source level
    args[VIRGL_CMD_RCR_SRC_X] = glyph->atlas_x; // Source X (Where char is in atlas)
    args[VIRGL_CMD_RCR_SRC_Y] = glyph->atlas_y; // Source Y
    args[VIRGL_CMD_RCR_SRC_Z] = 0; // Source Z
    args[VIRGL_CMD_RCR_SRC_W] = glyph->width; // Width of char
    args[VIRGL_CMD_RCR_SRC_H] = glyph->height; // Height of char
    args[VIRGL_CMD_RCR_SRC_D] = 1; // Depth

    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_RESOURCE_COPY_REGION, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)args, VIRGL_CMD_RESOURCE_COPY_REGION_SIZE);
	return AOS_TRUE;
}

void pyrion_destroy_font_virtio(struct pyrion_ctx* ctx, struct pyrion_font* font) {
	if (!ctx || !font) return;
	if (!ctx->valid || !ctx->usable || !ctx->device_set) return;
	if (font->res_id < RES_ID_FONT_BASE) return;
	if (ctx->controller_idx >= controller_count || !controllers) return;
	virtio_controller* kvc = &controllers[ctx->controller_idx];

    uint64_t slot = wait_for_free_buf(kvc);

	struct virtio_gpu_resource_detach_backing* db = (struct virtio_gpu_resource_detach_backing*)&kvc->cmd_buf[slot];
    memset(db, 0, sizeof(*db));
    db->hdr.type = VIRTIO_GPU_CMD_RESOURCE_DETACH_BACKING;
    db->resource_id = font->res_id;

    if (!virtio_submit_sync(kvc, slot, sizeof(*db), sizeof(struct virtio_gpu_ctrl_hdr))) return;

	slot = wait_for_free_buf(kvc);

    struct virtio_gpu_resource_unref* unref = (struct virtio_gpu_resource_unref*)&kvc->cmd_buf[slot];
    memset(unref, 0, sizeof(*unref));
    unref->hdr.type = VIRTIO_GPU_CMD_RESOURCE_UNREF;
    unref->resource_id = font->res_id;

    if (!virtio_submit_sync(kvc, slot, sizeof(*unref), sizeof(struct virtio_gpu_ctrl_hdr))) return;
}

aos_bool pyrion_upload_font_virtio(struct pyrion_ctx* ctx, struct pyrion_font font, struct pyrion_font* out) {
	if (!ctx || !out) return AOS_FALSE;
	if (!ctx->valid || !ctx->usable || !ctx->device_set) return AOS_FALSE;
	if (!font.atlas || !font.atlas_phys || font.w <= 0 || font.h <= 0 || font.valid) return AOS_FALSE;
	if (ctx->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	virtio_controller* kvc = &controllers[ctx->controller_idx];

	*out = font;
	if (!kvc->acceleration_present) {
		out->res_id = 0;
		out->valid = AOS_TRUE;
		return AOS_TRUE;
	}
	out->valid = AOS_FALSE;

	uint32_t res_id = ctx->ctx_id + RES_ID_FONT_BASE;
	out->res_id = res_id;

	uint64_t slot = wait_for_free_buf(kvc);

	// Create 3D Resource
	struct virtio_gpu_resource_create_3d* c3d = (struct virtio_gpu_resource_create_3d*)&kvc->cmd_buf[slot];
	memset(c3d, 0, sizeof(*c3d));
	c3d->hdr.type = VIRTIO_GPU_CMD_RESOURCE_CREATE_3D;
	c3d->resource_id = res_id;
	c3d->target = PIPE_TEXTURE_2D;
	c3d->format = PIPE_FORMAT_R8G8B8A8_UNORM;
	c3d->bind = PIPE_BIND_TRANSFER_READ | PIPE_BIND_SAMPLER_VIEW;
	c3d->width = font.w;
	c3d->height = font.h;
	c3d->depth = 1;
	c3d->last_level = 0;
	c3d->nr_samples = 0;
	c3d->flags = 0;
	if (!virtio_submit_sync(kvc, slot, sizeof(*c3d), sizeof(struct virtio_gpu_ctrl_hdr))) return AOS_FALSE;

	struct virtio_gpu_ctrl_hdr* resp = (struct virtio_gpu_ctrl_hdr*)&kvc->resp_buf[slot];
	if (resp->type != VIRTIO_GPU_RESP_OK_NODATA) {
		serial_printf("[VIRTIO:PYRION] Font upload failed (during create). resp type: 0x%x\n", resp->type);
		return AOS_FALSE;
	}

    // Attach Backing
    struct cmd_struct {
        struct virtio_gpu_resource_attach_backing att;
        struct virtio_gpu_mem_entry entry;
    } __attribute__((packed));
    slot = wait_for_free_buf(kvc);

    struct cmd_struct* cmd = (struct cmd_struct*)&kvc->cmd_buf[slot];
    memset(cmd, 0, sizeof(*cmd));
    cmd->att.hdr.type = VIRTIO_GPU_CMD_RESOURCE_ATTACH_BACKING;
    cmd->att.resource_id = res_id;
    cmd->att.nr_entries = 1;
    cmd->entry.addr = font.atlas_phys;
    cmd->entry.length = font.w * font.h * sizeof(uint32_t);
    if (!virtio_submit_sync(kvc, slot, sizeof(*cmd), sizeof(struct virtio_gpu_ctrl_hdr))) return AOS_FALSE;

    resp = (struct virtio_gpu_ctrl_hdr*)&kvc->resp_buf[slot];
    if (resp->type != VIRTIO_GPU_RESP_OK_NODATA) {
        serial_printf("[VIRTIO:PYRION] Font upload failed (during attach_backing). resp type: %x\n", resp->type);
        return AOS_FALSE;
    }

    // Attach to ctx
    slot = wait_for_free_buf(kvc);

    struct virtio_gpu_ctx_resource* link = (struct virtio_gpu_ctx_resource*)&kvc->cmd_buf[slot];
    memset(link, 0, sizeof(*link));
    link->hdr.type = VIRTIO_GPU_CMD_CTX_ATTACH_RESOURCE;
    link->hdr.ctx_id = ctx->ctx_id;
    link->resource_id = res_id;
    if (!virtio_submit_sync(kvc, slot, sizeof(*link), sizeof(struct virtio_gpu_ctrl_hdr))) return AOS_FALSE;

    resp = (struct virtio_gpu_ctrl_hdr*)&kvc->resp_buf[slot];
    if (resp->type != VIRTIO_GPU_RESP_OK_NODATA) {
        serial_printf("[VIRTIO:PYRION] Font upload failed (during attach). resp type: %x\n", resp->type);
        return AOS_FALSE;
    }

    // Transfer to Host
    slot = wait_for_free_buf(kvc);

	struct virtio_gpu_transfer_to_host_3d* t = (struct virtio_gpu_transfer_to_host_3d*)&kvc->cmd_buf[slot];
	memset(t, 0, sizeof(*t));
	t->hdr.type = VIRTIO_GPU_CMD_TRANSFER_TO_HOST_3D;
	t->hdr.ctx_id = ctx->ctx_id;
	t->resource_id = res_id;
	t->w = font.w;
	t->h = font.h;
	t->d = 1;
	if (!virtio_submit_sync(kvc, slot, sizeof(*t), sizeof(struct virtio_gpu_ctrl_hdr))) return AOS_FALSE;
	
    resp = (struct virtio_gpu_ctrl_hdr*)&kvc->resp_buf[slot];
    if (resp->type != VIRTIO_GPU_RESP_OK_NODATA) {
        serial_printf("[VIRTIO:PYRION] Font upload failed (during transfer). resp type: %x\n", resp->type);
        return AOS_FALSE;
    }

	out->valid = AOS_TRUE;
    return AOS_TRUE;
}

aos_bool pyrion_rect_virtio(struct pyrion_ctx* ctx, uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
	if (!ctx) return AOS_FALSE;
	if (!ctx->valid || !ctx->usable || !ctx->device_set) return AOS_FALSE;
    if (ctx->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	virtio_controller* kvc = &controllers[ctx->controller_idx];

	if (x > ctx->viewport.x + ctx->viewport.width || y > ctx->viewport.y + ctx->viewport.height) return AOS_FALSE; // Out of Bounds
	
	if (!kvc->acceleration_present) {
        if (!ctx->fb_info.addr) return AOS_FALSE;
        fb_draw_rect(&ctx->fb_info, x, y, w, h, rgba_to_u32(r, g, b, a));
		ctx->flush_needed = AOS_TRUE;
        return AOS_TRUE;
    }

	uint32_t scissor_args[VIRGL_SET_SCISSOR_STATE_SIZE(1)] = {0};
    scissor_args[VIRGL_SET_SCISSOR_MINX_MINY(0)] = ((y & 0xFFFF) << 16) | (x & 0xFFFF);
    scissor_args[VIRGL_SET_SCISSOR_MAXX_MAXY(0)] = (((y + h) & 0xFFFF) << 16) | ((x + w) & 0xFFFF);

    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_SET_SCISSOR_STATE, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)scissor_args, VIRGL_SET_SCISSOR_STATE_SIZE(1));
    
    float color[4] = {r / 255.0f, g / 255.0f, b / 255.0f, a / 255.0f};
    uint32_t args[VIRGL_OBJ_CLEAR_SIZE] = {0};
    args[VIRGL_OBJ_CLEAR_BUFFERS] = PIPE_CLEAR_COLOR;
	((float*)args)[VIRGL_OBJ_CLEAR_COLOR_0] = color[0];
	((float*)args)[VIRGL_OBJ_CLEAR_COLOR_1] = color[1];
	((float*)args)[VIRGL_OBJ_CLEAR_COLOR_2] = color[2];
	((float*)args)[VIRGL_OBJ_CLEAR_COLOR_3] = color[3];

    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_CLEAR, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)args, VIRGL_OBJ_CLEAR_SIZE);

    scissor_args[VIRGL_SET_SCISSOR_MINX_MINY(0)] = ((ctx->viewport.y & 0xFFFF) << 16) | (ctx->viewport.x & 0xFFFF);
    scissor_args[VIRGL_SET_SCISSOR_MAXX_MAXY(0)] = (((ctx->viewport.y + ctx->viewport.height) & 0xFFFF) << 16) | ((ctx->viewport.x + ctx->viewport.width) & 0xFFFF);

    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_SET_SCISSOR_STATE, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)scissor_args, VIRGL_SET_SCISSOR_STATE_SIZE(1));
	return AOS_TRUE;
}

aos_bool pyrion_blit_virtio(struct pyrion_ctx *ctx, uint32_t dst_res, uint32_t src_res, uint32_t width, uint32_t height){
	if (!ctx) return AOS_FALSE;
	if (!ctx->valid || !ctx->usable || !ctx->device_set) return AOS_FALSE;
	if (ctx->controller_idx >= controller_count || !controllers) return AOS_FALSE;
	virtio_controller* kvc = &controllers[ctx->controller_idx];
	
	if (!kvc->acceleration_present) return AOS_FALSE;

    uint32_t args[VIRGL_CMD_BLIT_SIZE] = {0};
    args[VIRGL_CMD_BLIT_S0] = VIRGL_CMD_BLIT_S0_MASK(PIPE_MASK_RGBA) | VIRGL_CMD_BLIT_S0_FILTER(PIPE_TEX_FILTER_NEAREST);

    // Scissor
    args[VIRGL_CMD_BLIT_SCISSOR_MINX_MINY] = 0;
    args[VIRGL_CMD_BLIT_SCISSOR_MAXX_MAXY] = (height << 16) | width;

    // DEST
    args[VIRGL_CMD_BLIT_DST_RES_HANDLE] = dst_res;
    args[VIRGL_CMD_BLIT_DST_LEVEL] = 0; // level
    args[VIRGL_CMD_BLIT_DST_FORMAT] = PIPE_FORMAT_R8G8B8A8_UNORM;
    args[VIRGL_CMD_BLIT_DST_X] = 0; // x
    args[VIRGL_CMD_BLIT_DST_Y] = 0; // y
    args[VIRGL_CMD_BLIT_DST_Z] = 0; // z
    args[VIRGL_CMD_BLIT_DST_W] = width;
    args[VIRGL_CMD_BLIT_DST_H] = height;
    args[VIRGL_CMD_BLIT_DST_D] = 1;

    // SRC
	args[VIRGL_CMD_BLIT_SRC_RES_HANDLE] = src_res;
    args[VIRGL_CMD_BLIT_SRC_LEVEL] = 0; // level
    args[VIRGL_CMD_BLIT_SRC_FORMAT] = PIPE_FORMAT_R8G8B8A8_UNORM;
    args[VIRGL_CMD_BLIT_SRC_X] = 0; // x
    args[VIRGL_CMD_BLIT_SRC_Y] = 0; // y
    args[VIRGL_CMD_BLIT_SRC_Z] = 0; // z
    args[VIRGL_CMD_BLIT_SRC_W] = width;
    args[VIRGL_CMD_BLIT_SRC_H] = height;
    args[VIRGL_CMD_BLIT_SRC_D] = 1;

    pyrion_push_virgl(ctx, VIRTIO_VIRGL_CCMD_BLIT, VIRTIO_VIRGL_OBJECT_NULL, (uint32_t*)args, VIRGL_CMD_BLIT_SIZE);
	return AOS_TRUE;
}
